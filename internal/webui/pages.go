...
const deviceProfilePreviewScript = `document.querySelectorAll("select[data-device-profile-control]").forEach(function(select){function update(){var option=select.options[select.selectedIndex];if(!option)return;var version=document.getElementById("profile-preview-version"),responsibilities=document.getElementById("profile-preview-responsibilities"),grants=document.getElementById("profile-preview-grants");if(version)version.textContent=option.value||"—";if(responsibilities)responsibilities.textContent=option.dataset.responsibilities||"—";if(grants)grants.textContent=option.dataset.grants||"—"}select.addEventListener("change",update);update()});`
...
func shell(d Deps, title, body string, isAuthed bool, evidence ...View) string {
	...
	if strings.Contains(body, "data-device-profile-control") {
		pageScripts += `<script>` + deviceProfilePreviewScript + `</script>`
	}
