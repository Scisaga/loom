...
const deviceInventoryLiveScript = `(function(){var summary=document.querySelector("[data-device-live-summary]"),list=document.querySelector("[data-device-live-list]");if(!summary||!list||!("WebSocket" in window))return;var socket=null,retry=1000,retryTimer=0,stopped=false;function state(label,tone){var target=document.querySelector("[data-device-live-state]");if(!target)return;target.className="device-live-state tiny sp "+tone;target.replaceChildren(document.createElement("span"),document.createTextNode(label));target.firstChild.className="dot"}function apply(html){var scroll=list.querySelector(".clients-table-scroll"),left=scroll?scroll.scrollLeft:0,active=document.activeElement,row=active&&active.closest?active.closest("[data-device-id]"):null,focusID=row?row.dataset.deviceId:"",focusLabel=active&&active.getAttribute?active.getAttribute("aria-label"):"",parsed=new DOMParser().parseFromString(html,"text/html"),nextSummary=parsed.querySelector("[data-device-live-summary]"),nextList=parsed.querySelector("[data-device-live-list]");if(!nextSummary||!nextList||parsed.querySelector("script,style,iframe,object,embed"))throw new Error("invalid inventory fragment");summary.replaceWith(document.importNode(nextSummary,true));list.replaceWith(document.importNode(nextList,true));summary=document.querySelector("[data-device-live-summary]");list=document.querySelector("[data-device-live-list]");var nextScroll=list.querySelector(".clients-table-scroll");if(nextScroll)nextScroll.scrollLeft=left;if(focusID&&focusLabel){list.querySelectorAll("[data-device-id]").forEach(function(candidate){if(candidate.dataset.deviceId!==focusID)return;candidate.querySelectorAll("[aria-label]").forEach(function(control){if(control.getAttribute("aria-label")===focusLabel)control.focus()})})}}function connect(){if(stopped)return;state("Connecting…","dim");var archived=new URLSearchParams(location.search).get("archived")==="1",scheme=location.protocol==="https:"?"wss://":"ws://";socket=new WebSocket(scheme+location.host+"/api/control/device-inventory/live"+(archived?"?archived=1":""));socket.onopen=function(){retry=1000;state("Live","ok")};socket.onmessage=function(event){try{var message=JSON.parse(event.data);if(message.type==="inventory"){apply(message.html);state("Live","ok")}else if(message.type==="error"){state("Update unavailable","warn")}}catch(error){state("Update unavailable","warn")}};socket.onerror=function(){socket.close()};socket.onclose=function(){if(stopped)return;state("Reconnecting…","warn");clearTimeout(retryTimer);retryTimer=setTimeout(connect,retry);retry=Math.min(retry*2,10000)}}window.addEventListener("beforeunload",function(){stopped=true;clearTimeout(retryTimer);if(socket)socket.close()});connect()})();`
...
.device-live-state{display:inline-flex;align-items:center;gap:5px;white-space:nowrap}.device-live-state .dot{width:6px;height:6px}
.clients-list-note{margin:0;padding:10px 16px 12px;border-top:1px solid var(--line)}
...
func shell(d Deps, title, body string, isAuthed bool, evidence ...View) string {
	...
	if strings.Contains(body, "data-device-live-list") {
		pageScripts += `<script>` + deviceInventoryLiveScript + `</script>`
	}
	if strings.Contains(body, `<svg class=topology`) {
		pageScripts += `<script>` + topologyInteractionScript + `</script>`
	}
	pageClass := "page-" + active
	return fmt.Sprintf(`<!doctype html><meta charset=utf-8><title>%s · LOOM</title><link rel=icon href="/favicon.svg?v=11" type="image/svg+xml">
