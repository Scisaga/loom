...
func DialOrder(endpoint wire.DataIngressEndpointV2, now time.Time, minimumGeneration int64) ([]wire.ListenerGenerationV2, error) {
	if !oneOf(endpoint.Transport, "hysteria2", "trojan_tls", "wireguard") {
		return nil, errors.New("[Linux] endpoint transport 无效")
	}
	...
	if len(preferred) != 1 {
		return nil, errors.New("[Linux] 当前可拨 endpoint 必须恰有一个 preferred generation")
	}
...
func ValidateMode(mode Mode, fixedEgressID string, authorizedEgressIDs []string) error {
	...
	case ModeDirect, ModeAuto:
		if fixedEgressID != "" {
			return errors.New("[Linux] Direct/Auto 禁止 fixed egress")
		}
	case ModeFixed:
		found := false
		...
		if !found {
			return errors.New("[Linux] 指定出口已移除或未授权；必须阻断，不能静默切 Auto")
		}
	default:
		return errors.New("[Linux] route mode 无效")
	}
	return nil
}
