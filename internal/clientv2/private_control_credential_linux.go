...
// 解封进 0600 state 的 credential 派生。外部 CLI 文件只保留给旧安装迁移（D124、D131）。
...
func installedLinuxPrivateControlContext(installation *EnrollmentInstallationV1,
	...
	if installation == nil {
		return linuxInstalledPrivateControlContext{}, false,
			errors.New("[D131 Linux private] enrollment installation 缺失")
	}
	...
	if err != nil || len(plaintext) == 0 ||
		...
		return linuxInstalledPrivateControlContext{}, true,
			errors.New("[D131 Linux private] installed private-control credential 编码/digest 无效")
	}
	...
	for _, encoded := range credential.InternalCARootsDER {
		...
		if decodeErr != nil || parseErr != nil || !bytes.Equal(certificate.Raw, der) {
			return linuxInstalledPrivateControlContext{}, true,
				errors.New("[D131 Linux private] installed internal CA root DER 无效")
		}
