...
func DecodeLinuxInviteDescriptor(raw []byte) (wire.InviteBootstrapDescriptorV2, error) {
	if len(raw) == 0 || len(raw) > MaximumLinuxInviteDescriptorBytes {
		return wire.InviteBootstrapDescriptorV2{}, errors.New("[Linux] Invite descriptor 大小无效")
	}
	...
	if err != nil || !bytes.Equal(canonical, raw) || descriptor.Schema != 2 {
		return wire.InviteBootstrapDescriptorV2{}, errors.New("[Linux] .loom-invite 必须是 exact canonical descriptor")
	}
	return descriptor, nil
}

// ReadLinuxInviteDescriptor 支持受控普通文件或标准输入。authority/QC/floor
// 仍由 proof verifier 建立；carrier 自身不成为 trust root。
func ReadLinuxInviteDescriptor(path string, stdin io.Reader) (wire.InviteBootstrapDescriptorV2, error) {
	if path == "-" {
		if stdin == nil {
			return wire.InviteBootstrapDescriptorV2{}, errors.New("[Linux] stdin 不能为空")
		}
	...
	if path == "" || filepath.Clean(path) != path {
		return wire.InviteBootstrapDescriptorV2{}, errors.New("[Linux] Invite file path 非规范")
	}
	...
	if file == nil {
		_ = unix.Close(fd)
		return wire.InviteBootstrapDescriptorV2{}, errors.New("[Linux] 无法建立 Invite carrier handle")
	}
	defer file.Close()
	info, err := file.Stat()
	if err != nil || !info.Mode().IsRegular() || info.Mode()&os.ModeSymlink != 0 ||
		info.Size() < 1 || info.Size() > MaximumLinuxInviteDescriptorBytes {
		return wire.InviteBootstrapDescriptorV2{}, errors.New("[Linux] Invite carrier 必须是有界普通文件")
	}
