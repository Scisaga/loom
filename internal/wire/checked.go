...
func CheckedAdd(left, right int64) (int64, error) {
	...
	if !value.IsInt64() {
		return 0, errors.New("[D104 wire] int64 加法溢出")
	}
...
func ParseTimeZ(value string) (time.Time, error) {
	...
	if err != nil || parsed.Format(time.RFC3339) != value || parsed.Location() != time.UTC {
		return time.Time{}, errors.New("[D104 wire] 时间必须是规范 RFC3339 UTC Z")
	}
	return parsed, nil
}
