...
// ACME 签发分开；renew policy 或 order 变化不能悄悄改变入口 SPKI（D122）。
...
// private key 仍只留在 KeyOwnerDeviceID 对应节点（D103、D122）。
...
func ValidateCertificateIdentityProjection(projection *CertificateIdentityProjectionV1) error {
	if projection == nil || projection.Schema != 1 ||
		...
		return errors.New("[D122 TLS] certificate identity projection 字段无效")
	}
	for _, endpointID := range projection.EndpointIDs {
		if !validIdentifier(endpointID, 128) {
			return errors.New("[D122 TLS] certificate endpoint ID 无效")
		}
	}
	for _, name := range projection.DNSNames {
		if !ValidFQDN(name) {
			return errors.New("[D122 TLS] certificate DNS name 必须是规范 FQDN")
		}
	}
	for _, hash := range []string{projection.KeyArtifactHash, projection.SPKIHash} {
		if _, err := ParseHash(hash); err != nil {
			return errors.New("[D122 TLS] certificate key artifact/SPKI hash 无效")
		}
	}
	return nil
}
...
func ValidateCertificateIntent(intent *CertificateIntentV1) error {
	if intent == nil || intent.Schema != 1 || intent.IssuanceGeneration < 1 ||
		...
		return errors.New("[D122 TLS] certificate intent generation/renew/overlap policy 无效")
	}
	...
	if err != nil || projectionHash != intent.IdentityProjectionHash {
		return errors.New("[D122 TLS] certificate intent 未绑定 exact identity projection")
	}
	csrDER, err := decodeCanonicalBase64URL(intent.CSRDER)
	if err != nil {
		return errors.New("[D122 TLS] certificate CSR 必须是规范 DER/base64url")
	}
	csrHash, err := HashBytes(DomainCertificateCSRDER, csrDER)
	if err != nil || csrHash != intent.CSRHash {
		return errors.New("[D122 TLS] certificate CSR hash 不匹配")
	}
	...
	if err != nil || !bytes.Equal(csr.Raw, csrDER) || csr.CheckSignature() != nil ||
		...
		return errors.New("[D122 TLS] certificate CSR identity/profile 无效")
	}
	publicKey, ok := csr.PublicKey.(*ecdsa.PublicKey)
	if !ok || publicKey.Curve != elliptic.P256() {
		return errors.New("[D122 TLS] public TLS identity 必须使用 P-256")
	}
	spki, err := x509.MarshalPKIXPublicKey(publicKey)
	if err != nil {
		return errors.New("[D122 TLS] certificate CSR SPKI 无法编码")
	}
	digest := sha256.Sum256(spki)
	wantSPKI := "sha256:" + hex.EncodeToString(digest[:])
	if wantSPKI != intent.IdentityProjection.SPKIHash {
		return errors.New("[D122 TLS] certificate CSR 与 identity SPKI 不匹配")
	}
	return nil
}
...
// 随机数，因而 reducer/proposal 可对同一输入得到相同 bytes（§12、D122）。
