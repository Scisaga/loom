...
// DPAPI LKG 内最后一刻 hydrate（D124、D131）。
...
func ValidateWindowsRuntimeArtifact(artifact *WindowsRuntimeArtifactV1) error {
	if artifact == nil || artifact.Schema != 1 || artifact.DeviceGeneration < 1 ||
		...
		return errors.New("[D131 Windows runtime] artifact header/files 无效")
	}
	...
	for index, ref := range artifact.CredentialRefs {
		if !validIdentifier(ref, 256) || index > 0 && artifact.CredentialRefs[index-1] >= ref {
			return errors.New("[D124 Windows runtime] credential refs 必须规范排序且唯一")
		}
	}
	...
	for _, file := range artifact.Files {
		if file.Content == "" || len(file.Content) > maximumWindowsRuntimeFile ||
			...
			return errors.New("[D131 Windows runtime] runtime file 内容无效")
		}
		total += len(file.Content)
		if total > 12<<20 {
			return errors.New("[D131 Windows runtime] runtime files 超过总预算")
		}
		...
		if err != nil || !bytes.Equal(canonical, []byte(file.Content)) ||
			...
			return errors.New("[D131 Windows runtime] runtime file 必须是 exact canonical JSON object")
		}
		if err := validateLinuxRuntimeJSONSecrets(object); err != nil {
			return errors.New("[D124 Windows runtime] public runtime 含非引用形式的敏感字段")
		}
	...
	if !slices.Equal(actual, artifact.CredentialRefs) {
		return errors.New("[D124 Windows runtime] credential_refs 未 exact 覆盖 runtime placeholders")
	}
	return nil
}
...
func validateWindowsRuntimeCABundle(value string) error {
	if value == "" || len(value) > 1<<20 || !utf8.ValidString(value) || strings.IndexByte(value, 0) >= 0 {
		return errors.New("[D124 Windows runtime] CA bundle 大小或编码无效")
	}
	rest := []byte(value)
	...
	for len(rest) > 0 {
		...
		if block == nil || block.Type != "CERTIFICATE" || len(block.Headers) != 0 {
			return errors.New("[D124 Windows runtime] CA bundle 必须只含规范 CERTIFICATE PEM")
		}
		...
		if err != nil || !bytes.Equal(certificate.Raw, block.Bytes) || !certificate.IsCA ||
			...
			return errors.New("[D124 Windows runtime] CA bundle 含无效或重复 CA certificate")
		}
		...
		count++
		if count > 16 {
			return errors.New("[D124 Windows runtime] CA bundle 超过 certificate 数量边界")
		}
		rest = next
	}
	if count == 0 || !bytes.Equal(canonical, []byte(value)) {
		return errors.New("[D124 Windows runtime] CA bundle 不是 exact canonical PEM")
	}
	return nil
}
