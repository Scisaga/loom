...
func recordOutcome(state State, selections map[string]string, result ProbeResult, now time.Time) State {
	...
	sort.Slice(state.Observations, func(i, j int) bool { return state.Observations[i].CandidateID < state.Observations[j].CandidateID })
	return state
}
...
func selectionStatuses(routes []clientmodel.RouteCandidate, observations []clientmodel.Observation,
	...
	for _, scope := range scopes {
		...
		for _, observation := range observations {
			...
			if observation.CandidateID == candidate.ID && observation.NetworkGeneration == generation && now.Before(until) {
