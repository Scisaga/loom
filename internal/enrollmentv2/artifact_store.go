...
// 且属于同一 Invite 的 immutable sealed artifact。
...
// 是服务端私有边界的一部分，不能把该目录挂到 public distribution。
...
func OpenSealedArtifactStore(root string) (*SealedArtifactStore, error) {
	if root == "" || !filepath.IsAbs(root) || filepath.Clean(root) != root {
		return nil, errors.New("[secret artifact] store root 必须是规范绝对路径")
	}
	...
	if err != nil || !info.IsDir() || info.Mode()&os.ModeSymlink != 0 || info.Mode().Perm() != 0o700 {
		return nil, errors.New("[secret artifact] store root 必须是 0700 实体目录")
	}
	return &SealedArtifactStore{root: root}, nil
}

// Put 先验证 ref/envelope exact binding，再以 O_EXCL 等价的 hard-link publish
// 原子固定 first bytes；同摘要只能重放完全相同的 canonical envelope。
func (store *SealedArtifactStore) Put(ref *wire.SecretArtifactRefV2,
	...
	if store == nil || ref == nil || envelope == nil || ref.SealedBlob == nil {
		return errors.New("[secret artifact] store put 输入不完整")
	}
	...
	if err := os.Link(temporaryPath, path); err != nil {
		...
		existingBody, _ := wire.MarshalCanonical(existing)
		if !bytes.Equal(existingBody, body) {
			return errors.New("[secret artifact] 并发 publish 产生不同 first bytes")
		}
		return nil
	}
...
func (store *SealedArtifactStore) Get(ciphertextDigest string) (wire.SealedSecretEnvelopeV1, error) {
	if store == nil {
		return wire.SealedSecretEnvelopeV1{}, errors.New("[secret artifact] store 未初始化")
	}
...
func (store *SealedArtifactStore) read(path, ciphertextDigest string) (wire.SealedSecretEnvelopeV1, error) {
	...
	if !info.Mode().IsRegular() || info.Mode()&os.ModeSymlink != 0 || info.Mode().Perm() != 0o600 ||
		...
		return wire.SealedSecretEnvelopeV1{}, errors.New("[secret artifact] stored envelope 不是 0600 有界普通文件")
	}
	...
	if statErr != nil || !os.SameFile(info, openedInfo) {
		_ = file.Close()
		return wire.SealedSecretEnvelopeV1{}, errors.New("[secret artifact] stored envelope 在读取时发生替换")
	}
	...
	if len(body) == 0 || len(body) > maximumSealedArtifactBytes {
		return wire.SealedSecretEnvelopeV1{}, errors.New("[secret artifact] stored envelope 大小无效")
	}
	var envelope wire.SealedSecretEnvelopeV1
	canonical, err := wire.DecodeStrict(body, maximumSealedArtifactBytes, &envelope)
	if err != nil || !bytes.Equal(canonical, body) {
		return wire.SealedSecretEnvelopeV1{}, errors.New("[secret artifact] stored envelope 不是 exact canonical wire")
	}
	digest, err := wire.SealedSecretEnvelopeHash(&envelope)
	if err != nil || digest != ciphertextDigest {
		return wire.SealedSecretEnvelopeV1{}, errors.New("[secret artifact] stored envelope digest 不匹配")
	}
	return envelope, nil
}
...
// 的 release 状态可以被 private service 读取。
func NewReleasedEnrollmentArtifactReader(transactions *Store,
	...
	if transactions == nil || artifacts == nil {
		return nil, errors.New("[secret artifact] release reader stores 不完整")
	}
	return func(ctx context.Context, clusterID, inviteID, ciphertextDigest string) (wire.SealedSecretEnvelopeV1, error) {
		...
		if clusterID == "" || inviteID == "" {
			return wire.SealedSecretEnvelopeV1{}, errors.New("[secret artifact] release identity 无效")
		}
		...
		if !found || validateDurableRecord(&record) != nil || record.State.ClusterID != clusterID ||
			...
			return wire.SealedSecretEnvelopeV1{}, errors.New("[Enrollment] artifact 尚未获得 completion release authorization")
		}
		...
		for index := range record.ResultArtifact.SecretArtifactRefs {
			...
			if candidate.SealedBlob != nil && candidate.SealedBlob.CiphertextDigest == ciphertextDigest {
				if exactRef != nil {
					return wire.SealedSecretEnvelopeV1{}, errors.New("[secret artifact] completed result 含重复 ciphertext digest")
				}
				exactRef = candidate
			}
		}
		if exactRef == nil {
			return wire.SealedSecretEnvelopeV1{}, errors.New("[secret artifact] ciphertext 不属于该 Invite result")
		}
