...
func activeEnrollmentProfile(t *testing.T) (wire.DeviceCertificateProfileStateV1, ed25519.PrivateKey) {
	...
	discoveryCertificate := approvalDeviceCertificate(t,
		wire.DeviceCertificateProfileStateV1{ProfileIntent: intent}, issuerKey,
		discoveryIdentity, "device", now.Add(time.Minute))
	parsedDiscovery, err := x509.ParseCertificate(discoveryCertificate)
	if err != nil {
		t.Fatal(err)
	}
	intent.ExtensionOrderOIDs = make([]string, len(parsedDiscovery.Extensions))
	for index := range parsedDiscovery.Extensions {
		intent.ExtensionOrderOIDs[index] = parsedDiscovery.Extensions[index].Id.String()
	}
