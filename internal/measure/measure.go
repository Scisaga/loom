// Package measure 保存和聚合带来源与目标的度量记录。
//
// 调度依赖度量提供候选链路的观测，度量也是状态解释与回放的依据。
// 没有真实质量数据就写调度引擎,权重只能靠猜,而且**无法验证阻尼参数是否
// 合适**:振荡是否发生,只有靠数据才看得出来。
package measure
...
type Measurement struct {
	...
	// Target 是这次测的目标地址。
	//
	// 同一候选对不同目标的可达性和延迟可能不同；不记目标会把无法比较的
	// 数据混在一起，回放时也无法拆分。
	...
	//
	// 首字节延迟不反映持续传输速率；优化大文件传输时不能用首字节排序
	// 代替吞吐指标。
