...
// 本文件定义服务、访问契约、调度声明和客户端凭据模型。
// 拓扑模型在 model.go,两者共用同一份 SSOT。

// 地址和出口是两个选择轴。它们互不相干,自由组合出四种情形,所以分成两个字段
...
// Objective 是单一优化目标。对外表达用约束 + 单一目标,
// 不用加权求和 —— 客户给不出权重。
...
// ConstraintKind 区分约束的性质。合规约束是"允许/不允许",
// 绝不能被高分挤掉,也绝不能被空候选集的自动回退绕过。
...
// Constraint 是过滤条件,不是评分项。
type Constraint struct {
	Kind ConstraintKind `yaml:"kind"`
	Expr string         `yaml:"expr"`
}

// Carrier 是模式 B 的承载方式。它由成员的访问契约是否同构决定,
...
// AccessContract 是转发前提:客户端已经发出的那一个请求,
...
type ServiceAddress struct {
	...
	// EgressCredentialRef 是出口服务器向它鉴权时用的凭据引用。
...
// AccessDeclaration 是选路的单位。
//
// 没有单个 adjustment_period —— 排序周期与微调周期分别配置,
// 合成一个值会强迫在"端点切太频"和"链路反应太慢"之间二选一。
type AccessDeclaration struct {
	ID   string `yaml:"id"`
	Name string `yaml:"name,omitempty"`

	// AddressAxis:from_request | class:<等价类 id>
	AddressAxis string `yaml:"address_axis"`
	// EgressAxis:any | pinned:<节点 id>
	...
	//
	// 不同目标可能具有不同的可达性；无关地址的低延迟不能证明候选能承载
	// 声明中的流量。objective 指定优化指标，probe_url 指定该指标对应的流量。
...
// Windows/Android 只复用签名配置语义，生命周期必须由各自宿主承担。
func (p Platform) UsesLinuxLifecycle() bool { return p == LinuxServer }

// MixedPort 是本机应用进入 Loom 的 mixed 入口。
//
...
type MixedPort struct {
	...
	// Services 为真时,这个入口按请求的 host 反查服务。这是正常的
...
// Credential 是接入节点表达"它要什么"的方式。
//
// 这里只有对秘密层的引用,没有明文。加入输入只授权中控已创建的
...
type Credential struct {
	...
	// Generation 是这份凭据的代次,轮换时 +1。
	//
	// **为什么需要它:** `cred/*` 是**跨节点共享**的 —— 一台服务器下线之后,
	// 它硬盘上还留着明文。不能轮换,"删除节点"就是不安全的。
	//
	...
	// RevokedAt 非空即已吊销。所有中继在下一轮询周期移除该 user。
