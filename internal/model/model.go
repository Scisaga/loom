...
type Tunnel struct {
	...
	// RetiredPorts 是这条隧道用过、**不该再回去**的端口。
	//
	// 起因是实测:hz01 ↔ ber01 在 61654 上被单向丢包丢了 15.8 小时,
	// 换成 61691 七分钟就通了。**换端口必须记住换掉了什么** —— 否则
	// 轮换两次就会转回那个已知不通的端口,而症状是"换了端口还是不通",
	// 跟真实原因毫不相干。
	//
	// 它同时是**记忆**和**盐**:分配器跳过这些端口,并且用退役个数改变
	// 哈希起点。只跳不换起点的话,线性探测会挑中退役端口的紧邻位
	// (61654 → 61655),而如果丢弃是按范围或邻近特征做的,那等于没换。
	//
