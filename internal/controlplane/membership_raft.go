...
// materialized roots；调用方不能把 leader 提供的 hash 原样返回。
...
// materialize 后原子安装新稳定 ControlSet，移除的本机身份同时停止投票。
func ApplyCommittedMembershipPrefix(ctx context.Context, storage *RaftStorage,
	...
	if storage == nil || ledger == nil || materialize == nil {
		return 0, errors.New("[joint apply] storage/ledger/materializer 不能为空")
	}
	applied := 0
	for {
		...
		if raft.LastApplied < 0 || raft.LastApplied >= int64(len(raft.Log)) {
			return applied, errors.New("[joint apply] committed prefix 坐标无效")
		}
		...
		case RaftRecordJointControlSet:
			if record.JointControlSet == nil {
				return applied, errors.New("[joint apply] Joint record 缺 payload")
			}
		...
		case RaftRecordHead:
			if record.Head == nil || record.Head.Body.Payload.HeadKind != "control_set_final" {
				return applied, errors.New("[joint apply] membership prefix 含非 Final Head")
			}
			snapshotHash, effectiveSSOTHash, err := materialize(ctx, *record.Head)
			if err != nil {
				return applied, fmt.Errorf("[joint apply] Final deterministic materialize 失败: %w", err)
			}
			ledgerState := ledger.Snapshot()
		...
		default:
			return applied, errors.New("[joint apply] committed prefix 含未知 record kind")
		}
