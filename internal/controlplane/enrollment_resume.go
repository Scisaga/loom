...
// 经 admin mTLS、base ACL 和 certified operation 授权的一次性响应。
...
func NewPrivateEnrollmentResumeControlService(overlayIP string, port int64,
	...
	if err != nil || address.String() != overlayIP || !address.IsPrivate() || port < 1 || port > 65535 ||
		...
		return nil, errors.New("[resume] private control tuple/dependencies/schema 无效")
	}
	...
	for kind, schema := range schemas {
		if kind == "" || strings.TrimSpace(kind) != kind || schema < 1 {
			return nil, errors.New("[resume] operation reader contract 无效")
		}
...
func (service *PrivateEnrollmentResumeControlService) ServeHTTP(writer http.ResponseWriter, request *http.Request) {
	if request == nil || request.Method != http.MethodPost || request.URL.Path != PrivateEnrollmentResumePath ||
		...
		writePrivateControlError(writer, http.StatusNotFound, "[resume] 路由不存在")
		return
	}
	if !service.matchesPrivateListener(request) || request.TLS == nil || request.TLS.Version != tls.VersionTLS13 ||
		...
		writePrivateControlError(writer, http.StatusForbidden, "[resume] admin private transport identity 被拒绝")
		return
	}
	if request.Header.Get("Authorization") != "" || request.Header.Get("Cookie") != "" ||
		...
		writePrivateControlError(writer, http.StatusBadRequest, "[resume] 请求格式被拒绝")
		return
	}
	...
	if err != nil || len(body) == 0 || len(body) > maximumControlRequestBytes {
		writePrivateControlError(writer, http.StatusBadRequest, "[resume] 请求正文无效或过大")
		return
	}
	canonical, err := wire.CanonicalizeStrict(body)
	if err != nil || !bytes.Equal(canonical, body) {
		writePrivateControlError(writer, http.StatusBadRequest, "[resume] 请求不是 exact canonical JSON")
		return
	}
	...
	if _, err := wire.DecodeStrict(body, maximumControlRequestBytes, &submitted); err != nil ||
		...
		writePrivateControlError(writer, http.StatusBadRequest, "[resume] 请求 identity 无效")
		return
	}
	...
	if err != nil || operationBody.Kind != "issue_enrollment_resume" || operationBody.PayloadSchema != 1 ||
		...
		writePrivateControlError(writer, http.StatusBadRequest, "[resume] operation/payload binding 无效")
		return
	}
	snapshot, err := service.read(request.Context())
	if err != nil {
		writePrivateControlError(writer, http.StatusServiceUnavailable, "[resume] authority 暂不可用")
		return
	}
	if submitted.ExpectedHeadHash != snapshot.Head.HeadHash || submitted.ExpectedHeadHash != operationBody.ParentHeadHash {
		writePrivateControlError(writer, http.StatusConflict, "[resume] expected head 已过期")
		return
	}
	...
	if err != nil {
		writePrivateControlError(writer, http.StatusForbidden, "[resume] 管理员授权被拒绝")
		return
	}
	result, err := service.commit(request.Context(), verified)
	if err != nil {
		writePrivateControlError(writer, http.StatusServiceUnavailable, "[resume] quorum 暂不可用")
		return
	}
	if err := verifyCertifiedControlOperation(&result, &submitted.Operation, request.TLS.PeerCertificates[0],
		...
		writePrivateControlError(writer, http.StatusInternalServerError, "[resume] certified operation 校验失败")
		return
	}
	...
	if err != nil {
		writePrivateControlError(writer, http.StatusConflict, "[resume] transaction/resume issuance 冲突")
		return
	}
	if err := validateResumeControlIssuerResult(&descriptor, &submitted.Authorization,
		...
		writePrivateControlError(writer, http.StatusInternalServerError, "[resume] issuer result binding 无效")
		return
	}
	...
	if err != nil {
		writePrivateControlError(writer, http.StatusInternalServerError, "[resume] response 编码失败")
		return
	}
...
func validateResumeControlIssuerResult(descriptor *wire.EnrollmentResumeDescriptorV1,
	...
	if descriptor == nil || authorization == nil || trustedTime.IsZero() ||
		...
		return errors.New("[resume] descriptor 与 admin payload 不匹配")
	}
	...
	if body.Mode != "resume_committed_claim" || body.ClusterID != authorization.ClusterID ||
		...
		return errors.New("[resume] capability 与 certified operation/descriptor 不匹配")
	}
	...
	if err != nil || expiresErr != nil || !effectiveTime.Before(expires) {
		return errors.New("[resume] certified operation/descriptor time 无效")
	}
	return nil
}
