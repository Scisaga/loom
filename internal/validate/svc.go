...
func checkServices(fs *findings, s *model.SSOT) {
	...
	for i := range svcs {
		...
		validAddresses := 0
		if svc.ID == "" {
			fs.add("服务", "service:(无 id)", "服务缺少 id")
			continue
		}
		if seenID[svc.ID] {
			fs.add("服务", where, "服务 id 重复")
			continue
		}
		seenID[svc.ID] = true
		...
		if !ok {
			fs.add("服务", where, "declaration 引用了不存在的声明 %q", svc.Declaration)
			continue
		}
...
// checkServicePorts 校验接入节点的入口语义。正常形态是一条中控托管
...
// checkCredentialRotation 检查凭据轮换的两步。
//
...
func checkCredentialRotation(fs *findings, s *model.SSOT) {
	for i := range s.Credentials {
		...
		if c.Generation < 0 {
			fs.add("轮换", where, "generation 不能为负:%d", c.Generation)
		}
