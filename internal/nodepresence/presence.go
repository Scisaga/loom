...
func Message(node, timestamp string) ([]byte, error) {
	if !validNodeID(node) {
		return nil, errors.New("[在线心跳] node 无效")
	}
	if len(timestamp) > len(time.RFC3339Nano)+8 {
		return nil, errors.New("[在线心跳] 时间文本过长")
	}
	if _, err := time.Parse(time.RFC3339Nano, timestamp); err != nil {
		return nil, errors.New("[在线心跳] 时间必须是 RFC3339")
	}
...
// 三字段包拉入整个 model/YAML 依赖；协议测试锁住同一边界。
...
func Assemble(node, timestamp string, signatureDER []byte) (Heartbeat, error) {
	...
	if len(signatureDER) == 0 || len(signatureDER) > 128 {
		return Heartbeat{}, errors.New("[在线心跳] P-256 签名长度无效")
	}
	return Heartbeat{
...
func Sign(node string, at time.Time, keyPEM []byte) (Heartbeat, error) {
	...
	if key.Curve != elliptic.P256() {
		return Heartbeat{}, errors.New("[在线心跳] 私钥必须使用 ECDSA P-256")
	}
	digest := sha256.Sum256(message)
	signature, err := ecdsa.SignASN1(rand.Reader, key, digest[:])
	if err != nil {
		return Heartbeat{}, fmt.Errorf("[在线心跳] 签名失败:%w", err)
	}
...
func Verify(heartbeat Heartbeat, publicKey *ecdsa.PublicKey, now time.Time, maximumAge time.Duration) (time.Time, error) {
	if publicKey == nil || publicKey.Curve != elliptic.P256() {
		return time.Time{}, errors.New("[在线心跳] 公钥必须使用 ECDSA P-256")
	}
	...
	if maximumAge <= 0 {
		return time.Time{}, errors.New("[在线心跳] 接收时效边界无效")
	}
	at, _ := time.Parse(time.RFC3339Nano, heartbeat.TS)
	age := now.UTC().Sub(at.UTC())
	if age < -MaximumClockLead {
		return time.Time{}, errors.New("[在线心跳] 时间来自未来")
	}
	if age > maximumAge {
		return time.Time{}, errors.New("[在线心跳] 已超过转发时效")
	}
	...
	if err != nil || base64.RawStdEncoding.EncodeToString(signature) != heartbeat.Signature ||
		...
		return time.Time{}, errors.New("[在线心跳] 签名编码无效")
	}
	digest := sha256.Sum256(message)
	if !ecdsa.VerifyASN1(publicKey, digest[:], signature) {
		return time.Time{}, errors.New("[在线心跳] 签名验证失败")
	}
	return at.UTC(), nil
}
...
func parsePrivateKey(keyPEM []byte) (*ecdsa.PrivateKey, error) {
	...
	for {
		...
		if block == nil {
			return nil, errors.New("[在线心跳] 私钥里没有可用 PEM 块")
		}
		rest = remaining
		switch block.Type {
		case "EC PRIVATE KEY":
			...
			if err != nil {
				return nil, errors.New("[在线心跳] EC 私钥格式无效")
			}
			return key, nil
		case "PRIVATE KEY":
			...
			if err != nil || !ok {
				return nil, errors.New("[在线心跳] PKCS#8 私钥不是 ECDSA")
			}
			return key, nil
		}
	}
