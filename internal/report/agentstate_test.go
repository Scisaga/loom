...
func TestAgentStateValidationRejectsWrongNodeAndStaleSelection(t *testing.T) {
	...
	got := strings.Join(validateAgentState(st, "demo-d", now), "\n")
...
func TestAgentStateValidationAcceptsFreshSelectorRead(t *testing.T) {
	now := time.Date(2026, 8, 26, 12, 0, 0, 0, time.UTC)
	st := &AgentState{Node: "demo-d", TS: now.Format(time.RFC3339), Selections: []AgentSelection{{
		Declaration: "d", Selector: "svc:d", Candidate: "cand:d:demo-b",
		UpdatedAt: now.Add(-time.Minute).Format(time.RFC3339),
	}}}
	if got := validateAgentState(st, "demo-d", now); len(got) != 0 {
...
func TestAgentStateForConfigRejectsMissingDeclarationAndNilHealth(t *testing.T) {
	now := time.Date(2026, 8, 26, 12, 0, 0, 0, time.UTC)
	cfg := &Config{
		Node: "demo-d", AgentState: "/var/lib/loom/agent-state.json",
		ExpectedComponents: ComponentVersions{Agent: "0.1.0"},
		ExpectedRoutes: []ExpectedRoute{
			{Access: "demo-d", Declaration: "d1", Chain: []string{"demo-b"}},
			{Access: "demo-d", Declaration: "d2", Chain: []string{"demo-c"}},
			// 其他接入节点的全网底图不能被误算成本机 declaration。
			{Access: "other", Declaration: "remote", Chain: []string{"demo-e"}},
		},
	}
	st := &AgentState{
		Node: "demo-d", TS: now.Format(time.RFC3339), ComponentVersion: "0.1.0",
		Selections: []AgentSelection{{
			Declaration: "d1", Selector: "svc:d1", Candidate: "cand:d1:demo-b",
...
func TestAgentStateForConfigAllowsLegacyHealthButNotEmptyNewState(t *testing.T) {
	now := time.Date(2026, 8, 26, 12, 0, 0, 0, time.UTC)
	cfg := &Config{
		Node: "demo-d", AgentState: "/state",
		ExpectedComponents: ComponentVersions{Agent: "0.1.0"},
		ExpectedRoutes:     []ExpectedRoute{{Access: "demo-d", Declaration: "d"}},
	}
	legacy := &AgentState{
