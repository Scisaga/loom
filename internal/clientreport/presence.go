...
// 心跳不复用 Observation 的 200/204 兼容语义：只有空正文 204 表示已接受（§16.4）。
func SendPresence(ctx context.Context, client *http.Client, endpoint string,
	...
	if err != nil || u.Scheme != "https" || u.Host == "" || u.User != nil ||
		...
		return Result{Err: errors.New("[§16.4 Windows 在线心跳] 无效 HTTPS 传输配置")}
	}
	if heartbeat == nil {
		return Result{Err: errors.New("[§16.4 Windows 在线心跳] 心跳缺失")}
	}
	if _, err := nodepresence.Message(heartbeat.Node, heartbeat.TS); err != nil {
		return Result{Err: errors.New("[§16.4 Windows 在线心跳] 心跳字段无效")}
	}
	...
	if err != nil {
		return Result{Err: errors.New("[§16.4 Windows 在线心跳] 网络请求失败")}
	}
	...
	if response.StatusCode == http.StatusNoContent {
		...
		result.Err = errors.New("[§16.4 Windows 在线心跳] 204 响应正文必须为空")
		return result
	}
	_, _ = io.Copy(io.Discard, io.LimitReader(response.Body, 4096))
	result.Err = fmt.Errorf("[§16.4 Windows 在线心跳] HTTP %d", result.Status)
	return result
}
