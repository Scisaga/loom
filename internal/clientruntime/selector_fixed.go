...
// 默认上网声明来自签名业务规则的结构，不能从 opaque selector 或 Service 名称猜测。
func windowsInternetSelector(sb *singBoxConfig) (string, []string, error) {
	...
	for _, rule := range sb.Route.Rules {
		...
		// 完整且唯一的受管入口边界才能扩大 Service 路由，不能波及探测或其他本地入口。
		if selector != "" || len(business) == 0 || len(rule.Inbound) != len(business) {
			return "", nil, errors.New("无法唯一识别完整业务入口的默认上网 selector")
		}
		for _, inbound := range business {
			if !slices.Contains(rule.Inbound, inbound) {
				return "", nil, errors.New("默认上网规则混合了业务和非业务入口")
			}
		}
...
// domain / domain_suffix / ip_cidr 是 OR，不能把域名误当作私网前缀的附加限制。
...
func windowsPrivateRouteScope(rule singBoxRule) (hasPrivate, onlyPrivate bool, err error) {
	...
	for _, value := range rule.IPCIDR {
		...
		if err != nil {
			return false, false, errors.New("上网规则的 IP 前缀无效，不能安全合并")
		}
		private := false
...
// 只把受管上网规则引用统一到同一 selector；DNS、探测认证及必要内网/引导规则保持原样。
func (p *WindowsSelectorPlan) fixedInternetConfig(sb *singBoxConfig, preference clientcore.Preference) (*agent.Config, error) {
	...
	if selector == "" || selector != p.internetSelector {
		return nil, errors.New("当前签名配置没有唯一的默认上网 selector，不能统一固定出口")
	}
	index := slices.IndexFunc(p.config.Declarations, func(d agent.Decl) bool { return d.Selector == selector })
	if index < 0 {
		return nil, errors.New("默认上网 selector 缺少可执行的完整路径测量声明")
	}
	...
	if len(d.Candidates) == 0 {
		return nil, errors.New("默认上网声明没有到所选出口的授权候选")
	}
	selectors := map[string]bool{}
	...
	for i := range sb.Route.Rules {
		...
		if private && rule.Outbound != selector {
			if !onlyPrivate {
				return nil, errors.New("私网与公网条件混合的独立 selector 规则无法安全合并到统一上网模式")
			}
			return nil, errors.New("独立私网 selector 无法合并到统一上网模式")
		}
		if len(rule.Inbound) == 0 {
			return nil, errors.New("上网 selector 规则缺少明确入口边界")
		}
		for _, inbound := range rule.Inbound {
			if !slices.Contains(business, inbound) {
				return nil, errors.New("上网 selector 规则混合了业务和非业务入口")
			}
		}
...
// 读取失败时也只显示当前模式实际参与决策的声明，不能复活已停用的 Service 行。
