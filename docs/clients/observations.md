...
先按已验证配置及已有证据应用并读回 selector，使请求立即按所选模式的授权结果路由；原子冻结本代入口
预算后异步执行这一批并行探测，整批结束后仅在 runtime、配置与网络代仍匹配时按当前最新模式/出口重算选择。
结果晚到、用户已切回 Direct、宿主已替换或 OS 网络代已变化时不能恢复旧模式或污染新代。
这一批无成功结果时保持未知和已经应用的合法路径，不追加采样；同代后续切换只复用 registry。
...
覆盖不清空这类记忆，也不重置网络代 registry。

入口 `rtt_ms` 必须来自实际 ICMP reply 的 RTT，不能把 ping 子进程从创建到退出的总耗时写入
该字段。无响应、输出无有效 RTT 或无法归属当前源接口/网络代时保持未知，不以零值补齐。
...
- Windows/Android 的 Direct 不探测；每底层网络代首次进入 Auto/指定出口时，才对当时冻结
  ...
  完整路径或阻塞启动；
- Android 初次启动及运行中 Direct→代理不等待入口批次；selector 先应用，批次完成后只向
  仍匹配的 runtime/配置/网络代按当前最新模式/出口提交结果，切回 Direct 或旧代迟到结果不能反向改模式；
- 入口 RTT 来自 ICMP reply，进程启动/退出开销不进入数值，无有效 RTT 保持未知；
