...
> 本节只定义迁移输入；源码接线见[实现对照](../development/implementation.md)，部署结果另按相关验收回执核对。
...
`certified`、`mirrored`、`applied` 与 `healthy` 也必须分开。源码能力由[实现对照](../development/implementation.md)说明，
