...
v2 迁移使用[控制面实施规程](../development/control-plane.md)，不向 v1 schema 填入新版字段。

## 开始前

- 阅读[协议契约](windows-reporting.md)、[Windows 构建入口](../../clients/windows/README.md)
  与[源码能力对照](../development/implementation.md)。实际部署及已有验收从 `.env` 指向的配置和
...
- 客户端选路只按[消费边界](observations.md#客户端消费边界)验证。
...
1. 未加入时，从当前中控正常生成有效 Windows 邀请，通过客户端二维码入口加入；已有身份则
   ...
   验证保存各阶段；`pending` 不算已完成。服务端延迟按[分段排查](../operations/enrollment-diagnostics.md)处理。
...
与日志进入忽略的 `deploy/evidence/`；公开[实现对照](../development/implementation.md)只更新源码和接线缺口。
