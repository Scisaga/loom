...
本节定义先后依赖，不维护完成勾选；源码接线见[实现对照](../development/implementation.md)，
...
选路按[客户端消费规范](observations.md#客户端消费边界)验证。
...
完整依赖与验收见
[从当前实现迁移](../protocols/control-plane/migration.md#从当前实现迁移)。
...
6. TUN/mixed 的 DNS、IPv4、IPv6、局域网与回环行为；同一业务 FQDN 在不同最终出口返回
   ...
   VPN 回环。具名远端局域网另按 [Local Network 专题](../proposals/local-network.md)的显式作用域与
   fail-closed 边界测试；
...
20. 数据入口轮换期间新旧端口重叠，新连接优先新代且可回退、旧连接排空；端口变化不
    改变出口/三态。Android 轮换沿用[客户端消费预算](observations.md#客户端消费边界)，
...
3. **已定方向：** v2 私有 control/config/report 通道使用分用途 mTLS，配置授权仍由 QC；
   ...
   KMS/HSM 仅见[后续强化计划](../proposals/key-protection.md)，不是私有通道的部署前提；
...
能力，必须定义相应模型、安全边界和选择理由，不能借用旧端口覆盖隐式实现。
