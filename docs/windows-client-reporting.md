...
见[控制面规范 §19](distributed-control-plane.md#19-从当前实现迁移)，不能把 v2 字段加进严格 v1 JSON。
源码入口和接线缺口见[实现对照](implementation.md)；实机流程见[验收规程](windows-client-reporting-prompt.md)。
...
- 200 正文损坏、超限或来源验签失败单独记录，不改写已经上报的本机健康。服务器观测的验证和
  Agent 消费只由[观测复用规范](client-observation-reuse.md)定义。
...
[客户端消费边界](client-observation-reuse.md#客户端消费边界)。不再保留旧 Service 健康 URL、
...
未签名请求的 `403` 只证明拒绝路径；细节按[验收规程](windows-client-reporting-prompt.md)记录。
