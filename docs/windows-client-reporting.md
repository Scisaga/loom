...
> **稳定契约：** Windows 保留原一分钟两签 Observation producer；本机检查不请求业务目标，
> 选路使用入口单次并行 ping 与服务器观测复用。已注册的 v1 Windows Loom 进程另以五秒周期
> 发送 `loom-presence-v1` 三字段心跳，在线 lease 为接收时间起十五秒。最新测试和部署情况见
> 忽略目录的当前状态。
> **边界：** 完整观测继续复用现有 `report.Observation`、`loom-attest-v5` 和
> `loom-selfcheck-v1`；心跳是独立协议，不刷新或替代任何 Observation 字段，也不改变 v2 wire。
>
...
只要 Windows Loom 进程已启动且身份已注册，就立即发送首个 loom-presence-v1 心跳，随后
每 5 秒向同一 report URL 的精确 presence=1 分支 POST；正文严格只有 node、ts、signature，
使用同一 DPAPI P-256 身份，且只接受空正文 204。心跳不读取数据面状态或流量。
...
measurements_sha256。两类 producer 分别串行生成严格递增 UTC 时间，拒绝重定向，失败等待各自
下一周期且日志脱敏。不要把心跳装入 Observation、增加第四个字段、兼容旧心跳响应或改变
self-check v2。Windows 邀请固定为
...
既有 gossip table，中控继续复用原有健康与配置版本判定。与之独立，只要已注册的 Windows
Loom 进程在运行，就立即并每 5 秒发送最小签名心跳；这条心跳只维持实时在线 lease。
...
独立在线心跳使用同一已验证 report 地址，但查询必须精确为 `presence=1`：

```text
POST https://report.demo-node.example/loom-client/report?presence=1
Content-Type: application/json

{"node":"demo-windows","ts":"<RFC3339Nano>","signature":"<base64 raw DER>"}
```

心跳请求不得携带 Observation、证书、token 或其他字段，只接受空正文 `204`；`200`、旧响应、
重定向和混合查询都失败关闭，不尝试回退到完整上报。它与 `observations=1` 请求使用不同 worker，
不等待 WireGuard/Hysteria 激活、流量、健康采集或服务端观测。

Observation 不使用 Bearer token 或 mTLS；认证来自两份签名附件中的节点证书和签名，以及
服务端 registry 中 `ready` enrollment 身份的精确 SPKI 绑定。心跳不重复携带证书，服务端以
`node` 找到同一当前 SPKI 后验证其独立签名。HTTP 客户端拒绝重定向，不记录私钥、完整证书、
正文或加入 token。
...
心跳每个 5 秒 tick 同样只尝试一次，失败后等下一 tick。它不使用 Observation 的
`Retry-After`/`200` 读取兼容分支，也不会触发 Observation worker。
...
独立心跳不属于上述 Observation 附件。它以 `loom-presence-v1` 域及长度前缀后的
`node`、`ts` 为原文，用同一 DPAPI P-256 私钥生成 ASN.1 DER ECDSA 签名，并以无填充
base64 写入 `signature`。服务端用 registry 中当前 `ready` Windows identity 的精确 SPKI 验签；
只接受签名时间单调推进的新包，重放不得刷新接收时间或唤醒列表。在线 lease 从可信包的服务端
接收时间起算 15 秒，不能直接使用客户端时钟。
...
- 在线心跳随已注册 Windows Loom 进程启动和停止，不随 activation、disconnect、恢复或流量
  开关；完整 Observation 仍只在 active workload 存在时生成。两者不得互相触发或刷新状态。
...
- `windows-desktop` 当前登记身份的严格三字段心跳可验签；夹带字段、篡改、撤销身份和混合查询
  被拒绝；重放不刷新按接收时间保存的 lease，也不刷新完整 Observation。
- 新心跳立即唤醒 Device inventory WebSocket；15 秒 lease 到期时，即使没有新报告或事件，
  已打开的列表也立即从 `Online` 更新为 `Stale`。
...
- 已注册但数据面未激活/已断开的进程仍立即并每 5 秒发送 `loom-presence-v1`；正文精确三字段、
  使用同一 DPAPI 身份、拒绝重定向且只接受空 `204`。一分钟 Observation 周期保持不变。

生产验收使用用户指定的 Windows Device，按正常用户流程执行：

1. 在中控取得有效加入二维码，用 Windows 客户端的粘贴、文件选择或拖放入口导入。
2. 由客户端完成加入、证书和 signed pull 验证，并实际启动数据面。
3. 检查进程启动后立即出现三字段心跳，随后约每 5 秒推进 `Last heartbeat`；停止进程后不再
   推进，并在 15 秒 lease 到期时由已打开的 Device inventory 自动显示 `Stale`。
4. 检查客户端自动生成的两签报告得到 `200` 观测数组（或旧服空正文 `204`）；
   读取扩展另核对真实来源观测已验签并进入当前 Agent，不能用空数组冒充复用成功。
5. 从中控核对该次加入产生的 Device ID、递增的 `ts`、last-seen 和实际 active snapshot。
   在 v1 验收中，只有 `applied` 与控制平面的 active signed snapshot 相同才表示配置已收敛；健康状态按真实证据验收。
6. 确认后续 60 秒 Observation 周期仍有更新，且五秒心跳没有改变 Observation 的 `ts`、
   `applied`、健康或观测数据。

本任务只改 Windows 客户端、共享三字段传输及必要的服务端验签/在线投影，不改 Android/Linux
producer 或部署配置，也不手工修改
