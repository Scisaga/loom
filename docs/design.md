...
> 1.464s 降到 0.066s。生产 Linux 已于 2026-08-30 收敛为 1083 一个中控托管
> 入口，旧 1080–1082 均已显式退役；固定 SG/DE 也由 Service 在中控选择，不再由
...
生产 Linux 已保留 1083 作为唯一入口，并显式退役旧 1080–1082；固定 SG/DE 改由
中控的 `Service → AccessDeclaration` 表达。旧端口没有被复用为新语义，因此依赖
错误端口的调用方会明确连接失败，不会在没有报错的情况下漂移出口。
