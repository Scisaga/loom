...
Android 窄屏使用固定底部“连接 / 配置 / 诊断”三页，各页独立滚动：连接页只突出 VPN 主操作
与当前路径，配置页承载入网、签名配置刷新和 Direct / Auto / 指定出口，诊断页承载网络证据、
可信上报、本机信任信息及仅 Debug 可见的 Direct fixture。连接页按相同实际签名 chain 对声明做
纯展示分组，默认最多展开两组摘要；逐声明详情只能在同一卡片内一次展开一项，以前后切换浏览，
不得另开弹窗或一次铺开完整列表。固定出口选择和待加入事务确认也必须在配置页内完成，不使用
模态弹窗。分组不得合并或改写决策、candidate 与观测；内联详情仍按实际 selector 读回展示每个
