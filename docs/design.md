...
Android 窄屏使用固定底部“连接 / 路由 / 设置”三页，各页独立滚动：连接页集中 VPN 主操作、
Direct / Auto / 指定出口及前往当前路径的入口；路由页承载当前路径、分段证据、网络状态与
可信上报；设置页承载入网、签名配置刷新、通知、本机信任信息及仅 Debug 可见的 Direct fixture。
未加入时连接页提供前往设置的入口，连接和模式选择保持禁用。三态按钮按实际文字宽度与字号
分配空间，标签完整单行；窄屏或大字体放不下时整组纵排，不截断或缩小文字，触摸高度至少 48 dp。
路由页按相同实际签名 chain 对声明做
纯展示分组，默认最多展开两组摘要；逐声明详情只能在同一卡片内一次展开一项，以前后切换浏览，
不得另开弹窗或一次铺开完整列表。固定出口选择在连接页、待加入事务确认在设置页内完成，不使用
模态弹窗。选择出口期间仍显示已确认模式，取消不改变偏好；切到 Direct / Auto 时收起出口列表。
未连接时当前路径显示空态，不把预选或旧连接路径呈现为实际路径。
分组不得合并或改写决策、candidate 与观测；内联详情仍按实际 selector 读回展示每个
声明、候选、签名 chain、原因、决策时间及逐连线同源观测。缺失显示 `—`，不能从中文原因反解析
数值，也不能用“业务健康”或整条路径 P50/P95 概括分段证据。
三页、关键交互与窄屏排版见 [Android SVG 原型](../assets/client/android/README.md)。
Android 的多配置目标与 Windows 一致：保存多份彼此隔离的加入身份与连接配置，每次只运行一份。
顶部配置入口定位到设置页列表；选择只切换查看，连接动作才先停止旧宿主再启动所选配置。
各页必须区分当前查看与正在连接的配置；新增、重命名、删除均为页内操作，新加入保持未连接，
运行中的配置须先断开再删除。候选、秘密、恢复事务、防回退状态及偏好按配置隔离；入口 probe
registry 仍按设备底层网络代共享，不因配置切换重新探测。当前 Android 存储实现仍为单配置，
多配置图是待实现目标，不能从 UI 原型推断已完成身份隔离。
