...
| 概念 | 定义 | 章节 |
|---|---|---|
| **两类节点** | 接入节点 / 服务器节点,**一个节点只承担一种**。目标地址不是节点 | §1 |
...
| 3 | **一个节点只承担一种角色**(两类走两个控制面);方向约束是**两端共同**决定的 | §1.3, §2.2 |
...
> ### 不变量
> **接入节点与服务器节点走两个不同的控制面。**
> 服务器由 Agent 拉取配置并持续收敛(§14、§15);接入节点靠 app 与一次性
> 链接分发(§18)。一台机器同时要两套分发机制,不是"能力的组合",是把两种
> 管理方式混在了一起 —— 校验器直接拒绝同时有两个块的节点。
...
Node                           # §1 —— Loom 管的机器。目标地址不在这里
  ...
  # 没有 capabilities 字段 —— 由哪个块存在推导,且只能存在一个(§1.3)。
