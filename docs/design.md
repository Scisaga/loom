...
| 3 | 节点能力是集合;方向约束是**两端共同**决定的,不是架构公理 | §1.3, §2.2 |
...
### 6.2.1 接入侧的协议由**客户端网络放行什么**决定

前两节讲的是"该用哪个更好"。接入这一跳还有一个更硬的前置问题:**上游的网络
让什么出去。**

实测发现过这样的网络:放行 UDP 53 与 UDP 123(内部 DNS 与 NTP),但**封掉
其余全部 UDP**。企业网与部分机房常见 —— 目的是封 QUIC,把流量逼回可审计的
TCP 代理。
...
> ⚠️ **不要默认 UDP 一定能出去。** §6.1 说跨受限链路默认 WireGuard,那是针对
> 服务器之间;接入节点所在的网络你往往控制不了,而 UDP 被整体封禁时,
> WireGuard 和 Hysteria2 会一起失效。

因此 `inbound_protocol` 是**按节点配置**的:同一套拓扑里,面向受限客户端的
服务器说 TCP,其余说 QUIC。这正是 §6 开头"协议按跳选择"的第二个理由 ——
第一个是快慢与隐蔽,第二个是**能不能通**。

**排查提示:** `dial tcp X:P: i/o timeout` 是包被丢(防火墙);
`connection refused` 是没人监听。两者的修法完全不同。
...
没有 `direction`、没有 `capabilities`、没有隧道 —— 那些都是节点才有的东西。
...
- `Node.capabilities` 里**没有 `target`** —— 出口是路径上的位置,由 `RouteCandidate.server_chain` 的最后一项决定(§1.1);
