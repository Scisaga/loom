...
> **Windows 客户端当前分工：** 只对授权入口做单次并行 ICMP，后段复用服务器现有
...
不保留已停用的 Service 行。目标地址不作为服务器节点；详情紧凑排列完整路径的最佳
质量、原因、读取时间及决策范围，当前质量与健康在卡片标题行显示，不重复占用详情行。
...
| **指定出口** | 全部接管的上网流量固定由所选最终节点出网；Windows 共用一条由 Agent 实测择优的完整路径，不再按 Service 分流 |
...
`Chain` 最后一跳等于所选出口裁剪，完整路径使用该声明原有的 targets、objective、
窗口、样本门槛和切换抑制参数。不同网站与 Service 的新连接因此共用实际读回的
...
Windows 使用 §16.1.2 的入口单次探测与已有服务器观测，不执行上述完整路径采样
