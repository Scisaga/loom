...
Android 采用适合窄屏的单列当前路径卡片：首页按相同实际签名 chain 对声明做
纯展示分组，默认最多展开两组摘要；逐声明详情只能在同一卡片内一次展开一项，以前后切换浏览，
不得另开弹窗或一次铺开完整列表。分组不得合并或改写决策、candidate 与观测；内联详情仍按实际
selector 读回展示每个声明、候选、签名 chain、原因、决策时间及
逐连线同源观测。缺失显示 `—`，不能从中文原因反解析数值，也不能用“业务健康”或整条路径
P50/P95 概括分段证据。首页只突出连接与三态模式；已加入信息压缩为次级卡片，网络诊断和
信任边界默认折叠，避免把运维明细铺成连接页长列表。
