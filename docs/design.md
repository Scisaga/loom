...
触摸区域至少 48 dp；放大文字时换行或纵排。原生界面遵循同一契约，安装与验收状态只见当前状态记录。
...
路由匹配轴；§7.2.1 的多连接配置用于隔离加入身份与网络，不用于识别应用账号。
...
一部分地址族或 NAT tuple 却被当作 generation 已就绪。runtime 同步返回已启动批次后，local verifier
对每个 exact bind tuple 做 TLS/QUIC handshake，生成绑定 prepared certified state 且最长一分钟有效的
opaque evidence；advertise 不能用任意 local hash 代替。external verifier 从该 opaque plan 导出精确
