...
| `ttft` / `throughput` / `cost` | ❌ Agent 拒绝执行,见下 |
...
`ttft` 要 L7 观测点(§16.2)、`throughput` 要批量传输探测、`cost` 要价格源
(§5.2)。拿 L4 首字节时间冒充这三个中的任何一个,产出的排序看着完全正常,
却在优化另一件事。渲染期就把这类声明挡在 Agent 配置外并报出原因,
