...
**Android 界面目标设计：** 以 Windows 当前功能界面为参照，将连接配置列表、所选配置的
状态与主操作、直连 / 自动 / 固定出口、逐服务当前选路排列在同一纵向滚动主界面。
选择配置只切换查看；所选配置与活动连接分别标明，其他配置的路径不能显示为所选配置的路径。
连接或切换连接须等待旧宿主、Agent 与数据面停止后才启动目标，同一时间只运行一个 VPN。
进度、错误、改名与删除始终绑定被操作的配置身份；运行配置不能删除。

直连与自动通过明确点击提交；固定出口先选择授权末跳，再确认，取消保留已确认偏好。
提交期间阻止重复操作，失败显示实际结果。Auto 按 Service 展示实际路径，FixedExit 只显示
一条统一上网路径，Direct 显示本机直连；均以实际 selector 读回为准。
详细信息在当前选路中展开，保留逐段协议、来源、原测量时间、决策原因与范围。
观测缺失保持未知，不拼成整条路径的实测质量；查看详情不触发探测。

添加采用名称与邀请输入表单，支持扫码、导入图片或文件、粘贴邀请；加入并保存后保持未连接，
既有连接继续运行。已开始的加入保留原身份与恢复信息，稍后继续仍使用原事务。
改名在列表原位编辑，删除确认明确配置名称且只移除本机数据。
身份、配置、秘密、偏好与恢复状态按配置隔离；入口 probe registry 按设备底层网络代共享，
配置切换不重新探测，迁移保留原身份。窄屏与大字体按文字所需空间换行或纵排，触摸区域至少 48 dp。
设计稿见 [Android 原型](../assets/client/android/loom-android.svg)。这只是目标界面；原生代码
仍是单配置和“连接 / 路由 / 设置”，多配置与新布局尚待实现。
