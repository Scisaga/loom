...
系统拖动、缩放、最大化、贴靠及关闭到托盘继续可用。请求管理员权限在后台执行，
...
刷新图标区域，静止状态不持续重绘。“当前选路”逐 Service 绘制实际 selector 读回映射
出的签名 chain，目标地址不作为服务器节点；详情显示完整路径的当前/最佳质量、健康、
原因和读取时间，不显示由分段 RTT 拼成的客户端质量。Direct 也须读回验证。
停止、取消、换代或读取失败不能把旧路径
