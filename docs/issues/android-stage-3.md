...
- 固定底部“连接 / 配置 / 诊断”三页各自承载主连接与当前路径、入网与选路、网络与信任诊断，
  各页独立滚动且底部导航保持可见；
- 当前路径在连接页按相同实际签名 chain 紧凑汇总，原卡片内一次展开一项详情并以前后按钮切换；
  路径浏览、固定出口选择和待加入确认不使用弹窗，仍逐声明、逐 candidate、逐连线显示完整证据，
  缺失保持未知；
- 生产受管配置真实 `200`、入口故障、固定出口、控制面离线及 Wi-Fi 换代有真机证据；
  蜂窝与 Wi-Fi/蜂窝切换由独立 [GitHub Issue #16](https://github.com/Scisaga/loom/issues/16)
  在具备 telephony 的真机上验收，不阻塞本 Issue；
