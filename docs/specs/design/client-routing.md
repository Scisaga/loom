...
宿主实现和实机结果只见[实现对照](../../implementation.md)。
...
RouteCandidate([§5.6](scheduling.md#56-决策位置由信息可得性决定))；只有 Linux 兼容覆盖才保留声明级 selector。

**`selector` 是手动开关:它自己不测速、也不切换。** 这是刻意的 —— sing-box
另有 `urltest` 会自动选最快的,但那会成为第二个互不知情的决策者,与 [§5.6](scheduling.md#56-决策位置由信息可得性决定)
"选路的决策者必须只有一个"冲突。
...
上 —— [§5](scheduling.md#5-调度) 的整套调度做完了也落不了地。**
...
因而极难诊断 —— 走代理的域名是交给出口解析的([§7.4](agent-runtime.md#74-环境变量代理的已知坑) 的 `socks5h`),根本
不经过本机。
...
([§5.8](scheduling.md#58-数据不足与候选集为空) 的 fail_closed),但 sing-box 自己去问解析器的那个连接也走 route 规则:
