...
目标机制；实际已覆盖到哪只见[实现对照](../../implementation.md)。
...
> **控制通道活着不等于配置是好的。** [§14.3](public-endpoints.md#143-拉取通道的保护) 的 Agent 拉取通道走中继反代,[§15.4](#154-制品与版本管理) 明确指出**该通道不依赖数据平面隧道健康** —— 这正是它适合下发制品的原因,但也意味着:**一个打挂了全部用户隧道的配置,照样能让 Agent 回连成功并被确认转正。**
...
> [实现对照](../../implementation.md)，下表定义目标能力。

| 要求 | 说明 |
|---|---|
| **版本显式钉住** | SSOT 中声明具体版本号,**永不使用 `latest`**。自动的是下载,不是升级决策 —— 上游一次不兼容发布可在一个轮询周期内打挂全部节点 |
| **配置与二进制绑定回滚** | 新版可能不认旧配置,旧版也可能不认新配置。**只回滚其一会得到起不来的节点**。快照必须同时钉住两者,[§15.2](#152-自动回滚仅-linuxserver-发布-canary-的跨节点目标态) 的回滚一起回 |
| **本地保留旧版本** | 回滚不能依赖网络下载。节点本地保留最近 N 个版本的二进制 |
| **制品镜像** | 节点不直连上游。控制平面镜像制品,经 [§14.3](public-endpoints.md#143-拉取通道的保护) 的中继反代通道下发 —— **该通道不依赖数据平面隧道健康**,正好适用 |
...
> **Android 不适用 Linux Agent 的 A/B 自更新机制** —— Android 宿主承载 [§7.5](agent-runtime.md#75-android-的调度承载) 的窄调度，
> 应用升级走固定包名、升级签名与受控分发渠道（见 [§17.5](fingerprints.md#175-性能代价与客户端耦合)）。这是平台更新机制差异，
> 不是允许静态选路或跳过 signed pull 的例外。
>
> 目标地址不是节点,自然也不涉及制品下发([§9](servers.md#9-目标地址))。
