...
**[§5.8](scheduling.md#58-数据不足与候选集为空) 引入的三类新告警:**
...
事件副本仍在被管系统里面。即使有多个 control Device，[§16.3](#163-告警) 那条约束仍然成立：
...
[实现对照](../../implementation.md)。设计告警策略前应先用事件历史估算频率，避免无依据地设置
噪声门槛。
...
无论独立 WireGuard UDP 入站探测是否启用（状态见[实现对照](../../implementation.md)），界面都必须
...
> **[§5.5](scheduling.md#55-调整周期与阻尼) 的阻尼参数无法在线调优。** 你不能拿生产流量试"阈值 20% 与 15% 哪个更好" —— 一次实验要跑很多天,而且期间的网络状况不可复现,两组结果不可比。
