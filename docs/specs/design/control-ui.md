...
因为有转述([§16.1.2](reporting.md#1612-按段量不按整条路线量))，普通节点可以展示自己持有的运行态；control 副本还会复制签名
...
不存在"对某台机器执行某某"这种旁路,而这正是 [§12](control-rendering.md#12-逻辑-ssot-与纯函数渲染) 想要的:节点上的所有配置
