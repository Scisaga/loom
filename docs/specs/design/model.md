...
| **公网接入 profile** | 直连 443、替代 HTTPS 端口，或经 NAT 映射的公开 listener（[§8.1](servers.md#81-职责)、[§14.3](public-endpoints.md#143-拉取通道的保护)） |
| **每条边的发起与 transport** | 这条边由哪端拨号，以及使用 WG、HY2 或经批准的 TCP transport（[§2](#2-发起方向可达性与-transport-是每条边的契约)、[§6](transports.md#6-隧道与协议)） |
...
2. 字段扁平放时,把 `direction` 写在纯接入节点上不报错也不生效 ——
   ...
   以为已经生效。分块之后它在 schema 层面就不成立([§12](control-rendering.md#12-逻辑-ssot-与纯函数渲染) 的严格解码在加载
   阶段即拒绝)。
...
> 对同一配置包里的重复路径直接报错；实现覆盖情况只见[实现对照](../../implementation.md)。

> **为什么不担心“多个配置源打架”：逻辑源只有一个。** certified SSOT → 渲染器是唯一的配置
> 来源。[§18](credentials-model.md#18-凭据与分发) 的加入输入只负责把中控已创建的 Device 与本机生成的身份绑定，并引导
...
> 生效；candidate render 只用于提交前校验，详见 [§11](control-rendering.md#11-铁律控制平面与数据平面分离)～[§12](control-rendering.md#12-逻辑-ssot-与纯函数渲染)。
...
标签描述**服务类型**。同一个服务类型下的多个地址,才有"选哪个"的问题([§4](#4-两个选择轴))。
...
> 一轮部署,快照历史里全是噪声,[§15.2](deployment.md#152-自动回滚仅-linuxserver-发布-canary-的跨节点目标态) 的自动回滚还会被反复触发。
> **配置里放稳定的域名,解析交给运行时。**([§12.1](control-rendering.md#121-纯函数的三个产物):运行时状态不进 SSOT)
>
> 代价是重解析是**静默**的 —— 修好了也没人知道。这个洞由度量补:Agent
> 上报每条隧道的握手时长([§16.1](reporting.md#161-采集什么) 的"握手成功率"),超阈值告警([§16.3](events-topology.md#163-告警))。
...
服务器数量可以为 **0**(直连),实践中 ≤2。**链上最后一台服务器就是这次的出口**([§1.1](#11-出口不是一种节点是路径上的一个位置))。
...
> ⚠️ 但这**不取消策略**。见 [§5.1](scheduling.md#51-策略裁剪候选集度量在候选集内选优):是否允许某条路径是策略问题,在允许的路径中选哪条才是度量问题。

### 3.2 组合爆炸必须裁剪

候选路径数 = 接入节点 × 服务器排列 × 候选地址。必须限制:最大跳数(建议 2);每个 (接入节点, 服务) 对预先裁剪候选集;**不对全排列探测,只探测候选集内的路径**。

**探测成本是系统的主要持续开销,必须显式管理。** 在推理场景下这个成本是真金白银(见 [§16.2](measurement.md#162-被动观测优先但被动能看到什么由观测点决定))。
...
| 由请求决定 | **出口钉死** | 固定出口 IP([§4.1](#41-为什么要钉死出口)) | 除出口外的服务器链 |
| **从等价类里选** | 全程可选 | 服务调度([§4.2](#42-为什么要在多个地址间选)) | 地址 + 服务器链,**合起来** |
...
> 钉死出口不限制前面几跳怎么选([§4.1](#41-为什么要钉死出口)),这是"互不相干"的确切含义。评分与排序的单位仍是
> `(服务器链, 地址)` 组合，证据可以来自该完整路径的明确观测点，或来自标明为估算的入口与
> 服务器分段；经广州到杭州地址实测的 TTFT 不能推成经北京到杭州的 TTFT。未覆盖维度保持
> unknown，不能把两个轴独立打分后相加，也不能为补齐每个组合强制客户端扫描完整路径([§5.6](scheduling.md#56-决策位置由信息可得性决定))。
...
在一组可互换的服务地址间调度。这是 Loom 价值最大的场景,也是 [§5.3](scheduling.md#53-目标函数约束式优于加权求和) 成本维度存在的原因。

**注意这些地址与 Loom 的拓扑无关。** 它们可能是你自建的推理服务,也可能是买的第三方 API —— Loom 一视同仁,都只是地址([§1](#1-两类节点加一类不是节点的东西))。区别只在能不能验证它们真的可互换([§4.3](#43-等价类标签不足以保证可替代性))、以及请求能不能原样换过去([§4.4](#44-访问契约换地址能不能只靠-l4-完成))。
...
[§4.3](#43-等价类标签不足以保证可替代性) 保证的是**输出等价** —— 两个地址算出来的东西一样。但要让流量真的能被换过去,还需要第二个条件:**客户端已经发出的那一个请求,原样送到新地址也能被接受。**
...
| 方式 | 前提 | Loom 的角色 | 适用 |
|---|---|---|---|
| **L4 直选**(默认) | 成员共用同一域名、同一 CA 签发的证书、同一套凭据与接口 | 出口服务器解析域名时**解析到选中的地址**,其余不动 | 自建服务的多机房部署([§4.2](#42-为什么要在多个地址间选)) |
...
**出口侧解析是 L4 直选的落地机制。** [§7.4](agent-runtime.md#74-环境变量代理的已知坑) 要求默认用 `socks5h://`,即由代理端而非客户端解析 DNS。这条规则在代理场景下是为了避免调度判断失真;在服务调度场景下它**直接成为地址选择的载体** —— 客户端始终请求 `llm.internal`,解析成哪个地址由 Loom 在出口服务器上决定,客户端无感知。

> **等价类的成员准入必须同时校验输出等价与契约同构。** 只满足前者的地址不能进 L4 直选的候选集。它需要 L7 网关承载,而网关是一个**独立组件**,与 L0–L3 正交(见 [§20](../../design.md#20-依赖驱动的构建顺序) 独立可选项)。
...
> 已实现和完成真机验收只见[实现对照](../../implementation.md)。
...
都能用的出口。[§8.2](servers.md#82-凭据即访问声明) 说服务器上的规则是准入校验 —— 只校验"允许走哪"而不
校验"允许去哪",这个准入是漏的。
...
| | 语义 | 数据平面做什么 | 度量单位 |
|---|---|---|---|
| **等价类**([§4.3](#43-等价类标签不足以保证可替代性)) | N 个地址**可互换**,挑一个 | **选地址** | (链, 地址) |
