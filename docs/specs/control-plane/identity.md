...
使用 [§7.1](consensus.md#71-内容寻址操作) RFC 6962 tree；`ca_profile_root` 对两类 profile 重算
...
[§11.3](bootstrap-capability.md#113-bootstrap-tunnel-capability) 的 `bootstrap_issuer_registry_root` 对每个 authorization ID 的最新代
...
由 leader 按“最合适”选择。`allowed_platforms[]` 按 [§11.2](enrollment-invite.md#112-invitecatalog-与紧凑二维码) platform enum、
...
`issuer_key_artifact_hash` 必须解析到 [§6.2](secret-artifacts.md#62-权威-secret-artifact) purpose=`ca_private_key` 的 exact ref，owner/issuer executor
