...
[§9.3](recovery.md#93-丢失-quorum) `RecoveryPrivateCustodyObjectV1` 的不可变 sealed artifact、真实 key-test 与离线
...
`intent_operation` 必须是 [§7.1](consensus.md#71-内容寻址操作) exact operation：
...
阈值，签名条目使用 [§9.3](recovery.md#93-丢失-quorum) 的 exact `RecoveryThresholdSignatureV1`。按 [§9.3](recovery.md#93-丢失-quorum) 计算
...
`effective_ssot_hash` 必须等于该唯一 reducer 输出；logical time/raft index/revision 按 [§7.4](consensus.md#74-raft-committed-log-与提交后-certified-qc)～[§7.5](consensus.md#75-时间语义)
...
ControlSet quorum 不可用时不存在“计划轮换”：旧 recovery threshold 仍可用则走 [§9.3](recovery.md#93-丢失-quorum) 的
