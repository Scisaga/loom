...
signature algorithm 和 wire 长度必须逐字段满足 [§6.1](identity.md#61-初始-adminca-与轮换) 的 exact admin profile；P-256 必须使用
...
满足纯函数约束。秘密随机值同样在 proposal preparation 按 [§6.2](secret-artifacts.md#62-权威-secret-artifact) 预生成，绝不能由 reducer 或
...
exact-version artifact ref 只进入 [§6.2](secret-artifacts.md#62-权威-secret-artifact) 规定的 control-private replicated object，并由公开 hash 承诺。
...
`ControlOperationV1`，三个非 admin variant 仅为 [§11.4](enrollment-transaction.md#114-内层-tlstokenkeystore-pop-与一次性提交) 明确定义的 admission-QC-authorized
...
`ordinary` 的 transition context 精确为 `{schema:1,kind:"ordinary"}`；其他 kind 分别使用 [§9.1](membership-transition.md#91-加入)、[§9.3](recovery.md#93-丢失-quorum)、
[§9.4](recovery-policy.md#94-recovery-policy-的计划轮换)、[§10.3](publication.md#103-v1--v2-不可逆-latch) 的 context。未知 kind、context 字段不匹配或额外字段全部拒绝。
...
旧 executor，不能替代 provider CAS 或本地 generation fencing，具体限制见 [§15](reconciliation-ui.md#15-外部副作用与租约)。
