...
显式 retention/tombstone 后再按 [§15](reconciliation-ui.md#15-外部副作用与租约) 清理，后续 proposal 不能用可变名字把它重新解释成新 secret。
...
后可为 [§11.3](bootstrap-capability.md#113-bootstrap-tunnel-capability) 的管理员授权 resume 保留到事务 `retry_not_after`，但过期 capability 本身不因此
...
任何 DNS/安装/cleanup 副作用前在 [§15](reconciliation-ui.md#15-外部副作用与租约) 的外部副作用账本中提交 private provider-state object；
不能只留进程内 handle。
...
`ArtifactAvailabilityReceiptLeafV1{schema,reporter_id,receipt_hash}` 使用 [§7.1](consensus.md#71-内容寻址操作) RFC 6962 规则；每个
...
`JCS(SecretArtifactRefV2)` 直接作为 [§7.1](consensus.md#71-内容寻址操作) RFC 6962 leaf 输入，空数组使用 RFC 6962 空树 root。
...
第二种 secret-ref 摘要算法；binding 的目标 wire 见 [§11.2](enrollment-invite.md#112-invitecatalog-与紧凑二维码)。
