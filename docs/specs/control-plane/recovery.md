...
`{key_id,recovery_key_pop_hash}` canonical leaves 使用 [§7.1](consensus.md#71-内容寻址操作) RFC 6962 树规则计算；缺 key、重复、额外
...
AuthorityProof key ID 按 [§6.2](secret-artifacts.md#62-权威-secret-artifact) 的 SPKI domain 重算，**不要求**等于从 raw key 计算的 recovery policy
`key_id`：generic PoP 用前者，`RecoveryKeyPossessionProofV1` 与 threshold signature 用后者定位同一
raw key，两种命名空间不得直接比较。immutable ref 不得为
`latest`、alias 或可覆盖路径；sealed ref 的 recipient refs 必须与 custodian refs 一一对应并满足 [§6.2](secret-artifacts.md#62-权威-secret-artifact)
...
按 [§6.2](secret-artifacts.md#62-权威-secret-artifact) strict SPKI 与映射重算。receipt public key必须严格解码为 32-byte
...
key ID 排序的 `RecoveryKeyCustodyLeafV1`，都使用 [§7.1](consensus.md#71-内容寻址操作) RFC 6962 规则；leaf 中 hash 必须由同行 exact
...
[§7.1](consensus.md#71-内容寻址操作) 的 JCS/UTF-8 规则，必须含 `schema=1` 和 `statement_type`，且不含签名数组、
...
`statement_type="emergency"`；计划轮换使用 [§9.4](recovery-policy.md#94-recovery-policy-的计划轮换) 的
...
`JCS(RecoveryGenesisPayloadV1)))`，其中 `frame` 是 [§7.1](consensus.md#71-内容寻址操作) 的长度前缀 domain framing；它只覆盖
...
或 policy hash 直接视为 recovery fork 并失败关闭。Device Merkle leaf 按[§10.1](publication.md#101-signed-current-v2)不重复放
