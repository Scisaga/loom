...
control 候选必须先按 [§11](enrollment-invite.md#11-enrollment邀请与报告) 完成 Enrollment、持有有效 Device identity，并已能通过永久 Loom
...
用户仍只维护一套逻辑模型，但任何
增删/换 key 都必须走 [§9](membership-transition.md#9-controlset-成员变化) 的 joint state machine，不能用普通 operation、CRDT 合并或手填第二
成员表绕过。
...
已有合格实现只见 [实现对照](../../implementation.md)，不得写死在目标协议。
