...
客户端必须按 [§7.4](consensus.md#74-raft-committed-log-与提交后-certified-qc) 重算完整 HeadEntry body 的 entry/head hash，并逐字段验证 tagged attestation、
...
得到不同 EndpointSet；每个[§10.1 Device leaf](#101-signed-current-v2) 的 `endpoint_set_hash` 承诺该 Device 的精确 bytes，
...
授权对象及 hash 必须从 effective state 重算，Membership 必须已完成 [§11.4](enrollment-transaction.md#114-内层-tlstokenkeystore-pop-与一次性提交) completion；endpoint
bundle 的每个 DataIngressEndpointSet exact bytes/hash 必须满足 grants 和职责且数组无缺漏，不能只返回
一个裸 endpoint hash；binding hash 必须等于 [§13](endpoints.md#13-endpointsetcatalog-与公网端口模型) 对该 set 的规范摘要。distribution/bootstrap set
...
`transition_proof_hash` 是 head entry 的普通被哈希字段：初始 v2 head 中它等于 [§10.3](#103-v1--v2-不可逆-latch) 的
...
三个摘要都使用 [§7.1](consensus.md#71-内容寻址操作) 的长度前缀 framing，首版固定为：
...
hash 或 QC。最终 `HeadEntryV2` 按 [§7.4](consensus.md#74-raft-committed-log-与提交后-certified-qc) 构造：
...
初始 control epoch 按 [§9.3](recovery.md#93-丢失-quorum) 固定为 0。三种 hash/framing 必须提供跨平台黄金向量。
...
`SHA-256(0x00 || JCS(BootstrapDeviceFloorLeafV1))`，内部节点、奇数分树和空树规则与 [§7.1](consensus.md#71-内容寻址操作)
