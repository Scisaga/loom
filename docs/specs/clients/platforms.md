...
签名、授权、floor 和 certified 门禁。源码接线及具体缺口见[实现对照](../../implementation.md)。
...
[入口预算](../../client-observation-reuse.md#客户端消费边界)，网络重连也不表示已验证配置失效。
...
执行入口与完成判定见 [Android 构建与交付规程](../../android-client-delivery-prompt.md)。
...
- 最小调度适配：按[客户端消费规范](../../client-observation-reuse.md#客户端消费边界)使用入口与
...
[客户端消费规范](../../client-observation-reuse.md#客户端消费边界)要求的 OS 网络代与入口测量接口；
不在宿主另建探测调度器或完整路径窗口。
