...
正常业务接线及缺口见[实现对照](implementation.md)；协议按[控制面规范](distributed-control-plane.md)。
本文不表示某环境已部署，也不自行授权初始化、换证、发布或停服。

命令中的 `<...>` 由 `.env` 所指部署配置解析；真实值与执行回执按
[本机部署说明](operations/local-deployment.md)保存，不写入本页。已有状态禁止重新 bootstrap。
...
- 浏览器授权不证明业务写入已取得 Raft/QC。`control_ping` 只验证该操作本身，完整业务交付
  仍按[实施规程](control-plane-implementation-prompt.md)验收。

## DNS 与部署输入

根 `.env` 中 `GANDI_PAT_TOKEN` 用于 Gandi LiveDNS 和 ACME DNS-01，由受保护进程加载；
声明检查、scope、代理和公网地址发现边界见[本机部署说明](operations/local-deployment.md)。
...
材料保管、算法、证书链及验收边界统一见[管理员证书规则](operations/admin-certificates.md)。
...
后续导出使用原路径。整组内容和验证边界见[证书规则](operations/admin-certificates.md)。
...
本规程。备份输入、原子发布和范围检查见[本机部署说明](operations/local-deployment.md)。
