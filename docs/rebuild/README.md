...
- Windows 原生功能仍不属于本轮最小重建；其源码、构建和虚拟化原生执行留在当前
  服务器，按 [Windows 11 测试虚拟机](../windows-test-vm.md)使用同机受限执行器，不维护
  第二份开发环境。交叉构建或虚拟机不能抵扣硬件相关的实体机终验；本分支保留现有
  UI 产品资产，除非当前任务明确授权，不改写其功能逻辑；
