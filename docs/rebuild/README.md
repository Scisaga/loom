...
3. Linux、Android 与当前获授权的 Windows 工作项从同一候选模型选择直连或中继，并以真实运行结果更新可用性；
4. 完整保留控制面 Web，并尽量保留 Android/Windows UI 产品资产；改由新的认证状态、运行时和观测投影供数。
...
- 保留 Android/Windows 的页面布局、品牌资源、profile/选路交互与实际状态展示；不连带保留旧网络状态机；
...
- Windows 原生功能默认不随核心五步扩张；Windows 客户端重建明确授权把既定客户端接入同一八概念模型。
  其源码、构建和制品判断仍留在当前服务器，按 [Windows 11 测试虚拟机](../windows-test-vm.md)使用同机受限
  执行器，不维护第二份开发环境。交叉构建或虚拟机不能抵扣 ARM64、真实睡眠、物理网络切换和显示硬件终验；
  除 Windows 客户端重建明确列出的入网、运行、报告、安装与三种既有交付形态外不扩张原生功能；
...
分支、issue 与发布等远端结果还必须满足[外部写入规则](../../AGENTS.md#外部写入)。
