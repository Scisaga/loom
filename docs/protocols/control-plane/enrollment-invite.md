...
### Invite、catalog 与紧凑二维码

以下对象都使用 [内容寻址操作](consensus.md#内容寻址操作) 的 canonical encoding、domain separator 和内容哈希。数组按文中键排序，
...
BootstrapEndpointCatalogV1
  ...
  bootstrap_ingress_set: BootstrapIngressEndpointSetV1 # exact wire 见 按用途拆分，不再使用公开 role=enroll/control
