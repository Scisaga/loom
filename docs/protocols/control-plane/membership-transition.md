...
每个 `peer_identity_artifact_hash` 必须解析到 [权威 secret artifact](secret-artifacts.md#权威-secret-artifact) purpose=`control_peer_identity` 的 exact
...
并在 [时间语义](consensus.md#时间语义) 可信时间窗内有效。control-peer TLS 固定 TLS 1.3，双方只接受当前 directory（Joint 时为
...
`{member_id,key_purpose,control_key_pop_hash}` leaves 使用 [内容寻址操作](consensus.md#内容寻址操作) 的 RFC 6962 规则计算。PoP 只证明
对应私钥确实存在，不授予成员资格；正常成员变化由
`ControlMembershipApprovalProofV1.new_control_key_possession_proofs` 携带并经其 hash 固定，紧急恢复
与首次 bootstrap 则分别由 [丢失 quorum](recovery.md#丢失-quorum)、[v1 → v2 不可逆 latch](publication.md#v1--v2-不可逆-latch) 的旧 authority 签名对象承诺该 root。
...
`admin_intent_operation` 必须是 [内容寻址操作](consensus.md#内容寻址操作) exact admin-signed operation：
...
同一 member 在同一侧永远只计一票。Joint config 与 Final head 分别使用 [Raft committed log 与提交后 certified QC](consensus.md#raft-committed-log-与提交后-certified-qc) 的 tagged QC，
...
leader 更替，只允许插入 [Raft committed log 与提交后 certified QC](consensus.md#raft-committed-log-与提交后-certified-qc) 的 current-term `RaftNoOpEntryV1` barrier，Final 坐标必须紧接真实
...
只能按 [时间语义](consensus.md#时间语义) 单调前进，`control_revision` 仍由 Final 的 raft index 映射；只有 authority 字段、
