...
## EndpointSet、catalog 与公网端口模型

### 按用途拆分，不再使用公开 role=enroll/control
...
[客户端消费边界](../../clients/observations.md#客户端消费边界)；Linux/server Agent 保持
[既有候选预算](../../architecture/measurement.md#探测预算有界而不是全探)。bootstrap registry 与
