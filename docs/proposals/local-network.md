...
>
> **规范依据：** [架构](../architecture/README.md)定义不变量、SSOT 与单一 Agent 决策边界；
> [客户端接入规范](../clients/README.md)定义加入、路由偏好与平台接管。
> 源码接线见[实现对照](../development/implementation.md)，运行事实按[部署证据](../operations/local-deployment.md)核实。
