...
约束由[权威 secret artifact](../protocols/control-plane/secret-artifacts.md#权威-secret-artifact)定义，
软件密钥与硬件保护的能力、成本和适用条件在下文分别说明。
本计划不构成私有入网、daemon 接线、存量迁移或旧路径删除的前置条件。
源码能力见[实现对照](../development/implementation.md)，具体后端及部署结果由[部署记录](../operations/local-deployment.md)核对。
...
本计划被明确启动前，架构规范、操作规程、issue 完成判定及部署脚本不得重新引入强制 KMS/HSM 门禁。
