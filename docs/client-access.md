...
邀请并填写本地名称，校验后再提交加入，不因打开或取消表单而留下空配置。表单不套居中
模态框或上传卡片，名称使用一个常规输入框，邀请使用说明文字与文件选择、粘贴按钮。
...
路径卡片直接排在主内容区，不再包一层总卡片。交互原型直接展示实际内容区，重命名只
增加条目内编辑框，添加只切换右侧表单，连接中只更新原摘要状态。三个 edition
