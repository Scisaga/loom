...
- `VpnService` 与常驻通知；
- 当前路径、连接状态、有限诊断与手动重连；
- 窄屏使用固定底部“连接 / 配置 / 诊断”三页：连接页只放主连接操作与当前路径，配置页放入网、
  签名配置刷新及三态选路，诊断页放网络证据、可信上报、本机信任信息和 Debug Direct；
  各页独立滚动，底部导航不随内容滚走；
- 连接页按相同实际签名 chain 汇总路径并默认折叠逐声明详情；展开后仍在原卡片内一次显示
  一项并以前后按钮切换。完整 selector 读回、逐连线证据、原因与原始时间不丢失；路径详情、
  固定出口选择和待加入事务确认均为页内操作，不使用弹窗；
