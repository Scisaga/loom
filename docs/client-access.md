...
**界面原型：** [三页总览](../assets/client/android/loom-client-home-misaka-v1.svg)、
[关键交互](../assets/client/android/loom-client-interactions-misaka-v1.svg)；
[多配置管理](../assets/client/android/loom-client-profiles-misaka-v1.svg)为待实现目标。
页面职责与窄屏规范见 [Android SVG 原型说明](../assets/client/android/README.md)。

Android 当前存储仍为单份加入身份与配置；目标界面支持保存多份配置，配置列表位于设置页，
各页顶部可进入列表切换查看。切换查看不启动连接，显式连接才停止旧宿主并切到所选配置。
每份配置的身份、偏好、待加入事务与防回退状态必须隔离；设备层入口 probe registry 仍跨配置
复用。同一时间只运行一个 `VpnService`，不构成只能保存一个配置的限制。
...
- `VpnService` 与常驻通知；
- 当前路径、连接状态、有限诊断与手动重连；
- 窄屏使用固定底部“连接 / 路由 / 设置”三页：连接页放 VPN 主操作、三态选路及路径入口；
  路由页放实际路径、分段证据、网络状态与可信上报；设置页放入网、签名配置刷新、通知、
  本机信任信息和 Debug Direct。各页独立滚动，底部导航不随内容滚走；
- 三态按钮标签保持完整单行，按实际字体测量宽度；不足时整组纵排，不缩字或截断，触摸高度至少
  48 dp。未加入时连接页提供前往设置的入口，连接与模式操作保持禁用；
- 路由页按相同实际签名 chain 汇总路径并默认折叠逐声明详情；展开后仍在原卡片内一次显示
  一项并以前后按钮切换。完整 selector 读回、逐连线证据、原因与原始时间不丢失；路径详情、
  固定出口选择和待加入事务确认分别位于连接、设置页，不使用弹窗。待选出口不冒充已生效模式，
  取消或切到 Direct / Auto 时收起列表；断开时当前路径显示空态；
