...
| 平台 | 流量接管 | 客户端形态 | 结论 |
|---|---|---|---|
| Windows 桌面 | TUN 兜底 + mixed 精确控制 | Windows Service + 托盘界面 | 需要薄客户端，不重写网络核心 |
| Linux 桌面 | TUN 兜底 + mixed 精确控制 | Loom + sing-box 二进制分发包 | 不开发独立客户端 UI |
| Linux 服务器 | 仅 mixed，由进程显式使用 | Loom + sing-box 二进制分发包 | 不开发独立客户端程序 |
| Android | `VpnService` TUN | Android App，内嵌 sing-box | 要完整能力就必须开发 App |
...
| 组件 | 职责 | 不负责 |
|---|---|---|
| 平台宿主 | 权限申请、前后台生命周期、服务启停、通知 | 候选生成与授权判断 |
...
systemd 服务显式接入。不同端口可绑定不同访问声明，实现并发而无全局切换。
...
Linux 桌面复用服务器组件，但允许 TUN 作为不支持代理程序的兜底，并保留多个
mixed 端口用于按程序、按声明精确控制。
...
- TUN 用于系统流量兜底，mixed 用于明确设置代理的程序；
- 设备密钥和凭据使用 DPAPI/CNG 保护；
- 配置与状态放入 ProgramData，不写入用户下载目录；
- 安装器负责服务注册、TUN 驱动依赖、卸载与恢复；
- 程序和安装器必须经过 Windows 代码签名。

首期可先交付 Windows Service + CLI 状态命令，托盘 UI 后补。若首期只做 mixed，
必须在产品状态中明确“非全局接管”，不能显示成设备已完全受 Loom 管理。
...
mesh；需要访问 mesh 内网时，由被授权的 Loom 服务器代为转发。首期每台 Android
设备只绑定一条默认访问声明和一份设备凭据，避免在没有 mixed 端口的情况下产生
含糊的多凭据选择。
...
管理员在控制中心为明确的平台和访问声明创建注册邀请。邀请包含：
...
- Linux server 的注册、pull、mixed、Agent、report、回滚形成安装流程；
...
1. Windows 首个可交付版本是否必须同时包含 TUN，还是允许明确标注的 mixed-only
   预览版；
2. Windows 安装器与代码签名证书的持有、轮换和 CI 签名方式；
3. Android 分发使用应用商店、企业 MDM 还是私有签名 APK；
4. mTLS 何时进入客户端稳态通道，以及设备证书的签发和吊销格式；
5. Android 是否长期只支持单一默认声明，还是提供受控的用户显式切换；
6. 客户端规模是否仍是少量自建固定设备；若面向多用户，设备库存和授权关系不能
