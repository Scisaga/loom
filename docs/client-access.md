...
**界面目标设计：** [Android 原型](../assets/client/android/loom-android.svg) 直接参照
[Windows 当前界面](../assets/client/windows/loom-client-windows-current.png) 与其功能实现。
连接配置列表放在主界面上部，后续区域显示所选配置的状态、连接操作、三态路由模式和当前选路。
点配置只切换查看；若另一份配置正在连接，单独标明它。明确点击“连接 / 切换连接”才启动目标，
同一时间只有一个 `VpnService`，切换先确认旧宿主停止，失败不自动启用其他身份。

加号进入邀请表单；加入成功保持未连接，已开始的加入可用原身份稍后继续。
列表支持原位改名；删除确认绑定具名的未运行配置，只删除本机身份与配置。
直连与自动点击提交，固定出口选定授权末跳后确认；当前路径与详细信息只读。
身份、配置、偏好、恢复事务与防回退状态隔离，设备层入口 probe registry 跨配置复用，
迁移保留已有身份。这是原型设计，原生 UI 仍为“连接 / 路由 / 设置”，存储仍为单份身份与配置。
