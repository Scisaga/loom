...
以发行说明为准，未签名的包继续标为预览版。本页规定交付边界，不构成已签名或已完成客户端
业务验收的证明；当前进度见[实施状态](../progress.md)。

## 范围与来源

签名范围为 Loom 自己维护的 Windows 客户端 EXE 和 MSI。提交给 SignPath 的产物
须来自当前服务器上的精确仓库提交和可回读的构建制品；源码、构建和制品判断不转移到第二套
开发环境。外部签名流程尚未启用，启用前必须证明签名请求与这份已验证制品的摘要绑定，并在
签名后重新验证文件，再生成下载校验清单。不能用另一次 runner 构建的文件替换已验收制品。
...
用户通过邀请选择加入的网络。Windows 客户端通过受限 bootstrap tunnel 和私有认证设备通道
交换加入信息、获取认证配置，并在数据面运行期间发送签名健康报告。报告包含设备身份、
