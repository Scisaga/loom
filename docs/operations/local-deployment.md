...
> 网络协议见[控制面规范](../protocols/control-plane/README.md)；源码能力见[实现对照](../development/implementation.md)。
...
历史流水账不承担现状查询；当前缺口在实现对照中原位更新，设计理由就近写入对应规范。
...
`go run ./cmd/loom backup` 并提供正常备份参数。外部路径上的身份和控制状态仍须按[控制面备份规程](control-plane.md)成组备份，
保存路径字符串不能代替备份文件内容。

恢复输入还包括原 SSOT、签名 keys、节点秘密与 PKI，以及源头不可变历史。回滚必须能同时
定位对应源头状态和制品，不能只留二进制或 `.env` 中的路径字符串。v1 的本地源头历史不得
进入公开分发树；目标复制属于 control-private 材料，按其受保护的备份边界保存。

## 代码发布

默认范围包括 Linux 生产代码和嵌入二进制的 Web UI / favicon。由所选精确、干净 commit 只构建一次制品，
本机和远端使用同一份字节。Android 双包另按[交付规程](../clients/android-delivery.md)执行。
