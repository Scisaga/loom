...
**类型：发行签名与隐私说明。** 适用于 Windows 发行物；网络协议信任由对应规范定义，
本页不声明签名服务已开通或某制品已签名。
...
完整 key-purpose 与迁移规则见[分布式控制平面设计](../protocols/control-plane/identity.md#信任域与密钥)。

## Privacy policy

用户通过邀请选择加入的网络。Windows 客户端与该网络控制面交换加入信息、
下载签名配置，并在数据面运行期间自动发送签名状态报告。v1 已注册进程另发独立在线心跳；
心跳只包含设备标识、时间与签名，不刷新完整报告。完整报告包含设备身份、时间、
已应用配置标识、本机健康结果和诊断类别；探测响应正文不进入报告。
设备私钥在本机生成并由 Windows DPAPI 保护。

应用流量与 DNS 按所加入网络的签名配置及用户的出口偏好处理。客户端健康检查只检查本机运行态；
入口测量和服务器观测消费遵守[客户端消费边界](../clients/observations.md#客户端消费边界)，不额外探测业务路径。
