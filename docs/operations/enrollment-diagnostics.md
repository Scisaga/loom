...
- **扫码加入：** 相机扫码与 Debug 的 ADB 模拟扫码均调用 `EnrollmentManager.importInvite`。
  二维码提供入网描述；客户端随后下载并验证 bootstrap 证明，在受限隧道内完成私有 Enrollment，
  再按 completion 回执下载运行配置、解封凭据并安装身份。模拟扫码不代替这些网络步骤。
- **存量迁移：** 导入 `.loom-migration`，验证并保留原身份和版本保护状态；不能作为新设备扫码加入的验收。
- **已加入设备更新：** 导入 `.loom-config`，消费新的认证配置；不能据此声称邀请、bootstrap 或首次 Enrollment 已接通。

已加入设备的认证配置文件从“配置 → 该连接的操作 → 导入配置更新”进入，VPN 停用时也可用。
导入先关闭旧 TUN、报告与 Agent，再经底层网络下载并验证制品；新运行配置与 selector
实际启动成功后才提交候选与版本保护状态。下载或启动失败时 VPN 保持停用，保留原身份与
已安装配置，允许再次导入。普通在线私有配置刷新仍通过已建立的 overlay 执行。
