...
所有新 Device 使用同一条 **Enrollment** 协议：管理员创建一次性邀请，设备安装
通用包后自行生成身份并 claim。二维码、`loom://`、邀请文件和服务器安装命令只是
同一邀请的不同载体，不是不同接入协议。
...
|---|---|---|
| 统一 Device | 建立统一 ID、库存读模型、`/devices` 与添加入口；旧 registry Client / SSOT Node 只作迁移适配 | 删除旧 `/clients`、`/nodes/add` 产品概念及兼容存储 |
| 统一 Enrollment | 新设备均使用邀请 → 本机密钥 → claim；服务器也走公开安装包和自注册 | 完成旧 SSH push 接入的迁移与移除 |
...
| 不可变 ProfileVersion | 邀请可钉住部署配置中的一个版本，并展示展开后的职责和 grants；不动态继承 | 多 Profile 管理、批量迁移仅在出现实际需求后设计 |
...
一个有迁移与回滚的协议改造实施，不能夹进 Enrollment 页面或 URL 筛选中零散上线。
...
材料和不含秘密的兼容性元数据。邀请 token、Device identity、证书、秘密、设备配置、
全局拓扑和运行证据不得进入 GitHub 或公开镜像。

包地址来自实际部署配置，代码不硬编码域名、IP、节点或供应商。Enrollment 页面从
配置生成二维码、邀请文件、URI 和服务器安装命令；安装命令下载公开通用包，再使用
同一邀请完成 claim，邀请 secret 不成为公开包 URL 的一部分。

### 5.2 E1 首次配置分发

当前 `readyBootstrap()` 直接返回 `SSOT.DistributionURLsFor(node)`，可能把 Enrollment
前可达的公网地址与接入后才能访问的 WireGuard 私网地址混在一起。E1 按以下规则修复：
...
E1 需要覆盖：更换部署地址无需改代码、私网/重复/非法 URL 排除、snapshot 不一致排除、
首选不可达时客户端使用备用地址、无候选 fail closed，以及日志不泄露邀请和密钥。
...
禁止包含无关 Device 清单、完整拓扑、其他设备凭据/配置、未授权 Service、邀请材料或
...
3. **统一 Enrollment：** Add Device 只创建一类邀请；服务器、Windows、Android 和
   Linux 使用同一 claim，平台由客户端报告；现有 SSH Add node 降为迁移入口；
4. **加入最小 ProfileVersion：** 邀请钉住部署声明的不可变版本，提交前展示展开结果；
   停止新增 Device 自动获得全部现有 `from_request` 声明，不做继承或 Profile 管理页；
5. **公开通用包：** 生成带签名/校验的通用包和服务器安装脚本，发布到部署配置声明的
   公开源；Enrollment 页面给出同一邀请的 QR、文件、URI 和可复制安装方法；
6. **修复首次 pull：** 落地 §5.2 的逐 URL 发布证据与选择函数，替换
   `readyBootstrap()` 当前直接继承节点地址的行为；
7. **收口旧入口：** `/clients` 与 `/nodes/add` 重定向到统一 Device 流程；完成迁移后
