...
数据必须明确标注，不能补画成绿色。现行兼容视图沿用原型的错位同心双环：
`reverse_only` 位于外圈，其余节点位于内圈；内圈优先本机，其他节点按 ID 排序，外圈旋转半步避免径向重叠。
这只是当前声明的可视化分组，不授权 SSH 连接，也不能代替每条边的发起方或可达性证明。
目标 v2 依据每条 LinkIntent 展示发起方、transport 与 purpose，不能从节点所在环推断所有链路的方向。
拓扑几何与交互沿用下方原型，渲染迁移不改变视觉结构。

线上拓扑不是嵌入的静态 SVG。每次渲染都从当前 `View.Nodes` / `View.Links` 生成：
新增节点按当前声明进入对应环并重新等角排列，新增 LinkIntent/声明边随数据一同出现。
