...
- 节点必须能**自治运行**;
- **排序结果也必须落地。** 控制平面离线时,节点沿用最后一次下发的 ranked list 继续本地选服务器链(见 [决策位置由信息可得性决定](scheduling.md#决策位置由信息可得性决定))。

### `control` 是动态节点能力，不是固定三台机器
...
[分布式控制平面专题](../protocols/control-plane/membership-model.md#controlsetquorum-与故障模型)。
...
理由是一致性约束:[只渲染被显式授权的 LinkIntent](transports.md#只渲染被显式授权的-linkintent) 的 N×M 矩阵两端必须严格对应;凭据到访问声明的映射必须在所有服务器上一致;候选集必须与实际存活的节点集合一致;混淆参数双端必须逐位相同。**这些人工维护必然出错。**
...
**节点私钥是显式例外。** [私钥不集中生成](identity.md#私钥不集中生成) 要求 WG 私钥只在节点本地生成、永不离开节点 —— 平台既然没有它,就渲染不出含 `PrivateKey = ...` 的完整文件。两条规则用**渲染配置 + 本地秘密覆盖层**调和:

| 层 | 内容 | 谁产生 | 进快照吗 |
|---|---|---|---|
| **渲染层** | 配置文件全文;私钥位置写成对本地文件的引用(wg-quick `PostUp` 读 `/etc/wireguard/node.key`) | 平台渲染 | ✅ 全文哈希 |
| **秘密层** | 私钥文件本身,**一台机器一把**(SSOT 里每节点只有一个公钥) | 节点本地生成 | ❌ 只记 `secret_generation` 与**公钥** |

WireGuard 身份按机器共用一把密钥；本机密钥使用消费者允许访问的受保护路径，
不能为了统一目录名而改变它的读取边界。

v1 配置包 hash 只覆盖本节点渲染文件，不混入全局 SSOT hash 或秘密；无关节点的变化不应
制造本节点配置漂移。manifest 与 detached signature 分开保存，验签使用收到的原始字节，
不通过重新序列化重建签名输入。v2 authority 与 proof 的额外约束见[发布协议](../protocols/control-plane/publication.md)。

纯函数性质因此保住:同一份 SSOT 在任何时候渲染出的字节完全相同,私钥不参与渲染,[幂等收敛](deployment.md#幂等收敛) 的哈希比对也只比对渲染层。
