...
> 获得，不能信任 HTTP Host、重定向或未签名配置。完整认证契约见[Windows v1 报告](../clients/windows-reporting.md)。
...
旧安装清单减去新计划得到删除集合，只能删除 Loom 管理的文件；先停止相关服务，再删除
文件，删除同样进入 previous 和回滚事务。首次安装允许清单缺失，清单损坏必须报错，不能
当成首次安装跳过清理。所有激活入口共享同一事务锁；崩溃恢复先处理未完成事务，再接收新安装。

安装顺序的理由：

**预检必须先于覆盖。** 配置非法会导致 sing-box 启动失败或崩溃循环；
在暂存目录验证，才能让拒绝发生时旧配置与服务仍然可用。
...
大制品下载同时限制无进展时间、由签名大小推导的总时限和读取字节数，防止持续滴流无限
占住更新事务。下载完成仍须验内容 hash；时间限制不能代替完整性验证。

publisher 进程心跳与最近发布结果分别记录。仅当失败晚于最近成功时才存在未恢复的发布错误；
长期没有配置变化不等于 publisher 失联，也不构成重复发布的理由。

### 程序与配置共同分发

v1 Agent 二进制的 SHA-256 进入 manifest，与配置一起受快照签名保护。二进制按内容寻址，
多快照引用同一份字节；配置回滚同时定位匹配的程序制品。

```text
dist/bin/<sha256>            ← 跨快照共享
dist/<id>/snapshot.json      ← manifest 引用程序 hash
```

更新按哈希验证、自检、保存旧程序、原子就位和服务启动验证执行；候选不可运行或不能读取
本机现有配置时拒绝安装。启动失败恢复旧程序与受影响服务，不能让节点失去获取修复的能力。
新版程序继续同轮安装、release 放行和发布器读取规则见下文。

### 分发面：公开证明与私有 Device view 分离
...
协议正文见[签名发布与 anti-rollback](../protocols/control-plane/publication.md)、
[私有 Device 配置与报告](../protocols/control-plane/device-services.md)、
[EndpointSet 与用途边界](../protocols/control-plane/endpoints.md)。公开镜像、DNS、URL 顺序和较大 revision
...
> [迁移完成条件](../protocols/control-plane/migration.md)删除。严格 reader
...
### Publisher/reconciler：提交与外部副作用分开
...
[外部副作用与租约](../protocols/control-plane/reconciliation-ui.md#外部副作用与租约)。租约只减少重复
