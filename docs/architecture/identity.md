...
> 证书分域与 recovery root 是目标态。是否仍运行 v1 只见[实现对照](../development/implementation.md)。
> [SSH 证书 CA 替代 authorized_keys](#ssh-证书-ca-替代-authorized_keys) 的 SSH User/Host CA 是运维目标设计，不是
> Device Enrollment 机制。完整迁移见
> [信任域与密钥](../protocols/control-plane/identity.md#信任域与密钥)。
...
| **admin 私钥** | 管理员终端/硬件；新签发为 P-256，TLS 与操作签名共用 | 仅验证完整签发链与 certified ACL；换证见 [初始 admin/CA 与轮换](../protocols/control-plane/identity.md#初始-adminca-与轮换) |
...
[权威 secret artifact](../protocols/control-plane/secret-artifacts.md#权威-secret-artifact) 执行。
KMS/HSM 仅列入[后续强化计划](../proposals/key-protection.md)，不构成当前上线依赖。
...
所以一台服务器下线之后,它硬盘上还留着全网在用的凭据 —— [节点生命周期:四个状态,加和删要对称](lifecycle.md#节点生命周期四个状态加和删要对称) 因此要求
"移除节点之后必须轮换它持有过的凭据"。
...
- [端到端流程、Invite 与二维码](../protocols/control-plane/enrollment-invite.md)；
- [Bootstrap capability、transport 认证和入口 ACL](../protocols/control-plane/bootstrap-capability.md)；
- [内层 TLS、PoP、reservation → issuance → approval → completion](../protocols/control-plane/enrollment-transaction.md)；
- [正式 Device 配置与报告](../protocols/control-plane/device-services.md)。

v1 严格 schema 不接受 v2 字段。版本替换须按[ 迁移完成条件](../protocols/control-plane/migration.md)
