...
| **可吊销** | 控制面拒绝读取/上报与数据面撤除凭据必须分别收敛；各平台源码接线见[实现对照](../development/implementation.md)，实际覆盖须核对验收回执 |
| **有效期** | 自带过期时间 |
...
[分布式控制平面专题](../protocols/control-plane/README.md)及其黄金向量为准。

```
Node                           # 两类节点,加一类不是节点的东西 —— Loom 管的机器。目标地址不在这里
  id, name, country, city, provider
  public_endpoint?, ssh_port?, nat_type, arch, os
  components?                  # 组件版本,缺省取全局默认(制品与版本管理)
  agent_last_seen, agent_version, status

  server?                      # 这个块存在 = 持有 server 能力(数据平面职责分块；`control` 是正交只读投影)
    direction                  # 仅 v1 兼容/迁移输入；目标态由每条 LinkIntent 表达(发起方向、可达性与 transport 是每条边的契约)
    public_data_ingress?       # 仅 v1 compatibility 客户端直拨开关；v2 单独无 authority(反连只是某条链路的实现)
    inbound_port               # 接受上游连接的端口(职责)
    inbound_protocol?          # hysteria2 | trojan(接入侧协议可按节点配置)
    egress_capable             # 能否作为出口出公网
    wg_public_key              # 节点上报,平台不持有私钥
    secret_generation          # 秘密层代次,只记代次不记私钥(纯函数的三个产物)
  ...
  access?                      # 这个块存在 = 持有 access 能力(数据平面职责分块；`control` 是正交只读投影)
    platform                   # android | windows-desktop | linux-server(平台差异)
    credentials[]              # 凭据即访问声明
  ...
  # 没有 capabilities 字段 —— 由哪个块存在推导；多个块并存合法(数据平面职责分块；`control` 是正交只读投影)。
  # 没有 target 能力 —— 出口是位置不是类型(出口不是一种节点,是路径上的一个位置)。
  # 没有 mesh_eligible —— 是否存在 overlay/data link 由 certified LinkIntent 决定(发起方向、可达性与 transport 是每条边的契约)。

ServiceAddress                 # 目标地址 —— 目标地址。不是节点,不参与渲染
  address                      # 如 https://llm-b.internal/v1
  service                      # 服务类型标签(目标地址带服务类型标签)
  access_contract              # 访问契约:换地址能不能只靠 L4 完成 转发前提:域名、证书 CA、凭据来源、
                               #      请求路径前缀、协议版本
  egress_credential_ref?       # 出口服务器向它鉴权用的凭据引用(出口鉴权凭据)
  region?, provider?           # 仅用于策略过滤与排障标注

EquivalenceClass               # 等价类:标签不足以保证可替代性 —— 可互换性的定义
  id                           # 如 llm:qwen3-32b-int8@openai-v1
  output_contract              # 等价类:标签不足以保证可替代性 输出等价:参数、上下文长度、流式行为
  carrier                      # l4_direct | l7_gateway | sdk(访问契约:换地址能不能只靠 L4 完成)
                               # l4_direct 要求所有成员 access_contract 同构
  observation_point            # 决定能产出哪些指标(被动观测优先,但被动能看到什么由观测点决定)
  price_source?                # 无则 objective: cost 不成立(评分输入分两类)
  verification_suite           # 定期校验用的测试请求集(两个契约都要校验)
  members[]                    # ServiceAddress 列表 —— 是地址,不是节点
  last_verified_at, failures[]

ReachEdge                      # 仅 bootstrap 阶段(可达性是图，不是成员资格)
...
Tunnel                         # v1 compatibility；迁移后投影成 LinkIntent
  ...
  obfuscation_param_set?
                               ...
                               # 同一网卡上无法共存(只渲染被显式授权的 LinkIntent)

LinkIntent                     # 目标态：每条边的唯一传输契约(发起方向、可达性与 transport 是每条边的契约、只渲染被显式授权的 LinkIntent)
...
RouteCandidate                 # 决策位置由信息可得性决定 —— 排序、选择与归因的唯一单位
  id                           # 下发 ranked list 与上报 Measurement 时引用它
  declaration_id
  server_chain[]               # 有序;最后一台就是这次的出口(出口不是一种节点,是路径上的一个位置)
  address?                     # 地址轴若"由请求决定"则为空
  eligible                     # 策略过滤结果(策略裁剪候选集,度量在候选集内选优),false 则不进排序
  score, rank                  # 排序产物 —— 运行时状态,不进 SSOT(纯函数的三个产物)
  sample_count, last_sample_at # 冷启动与过期判定(数据不足与候选集为空)

AccessDeclaration              # 两个选择轴 —— 访问声明,选路的单位
  id, name

  # 两个轴分别声明,不合成一个 mode(两个选择轴)
  ...
  constraints[]                # 合规、地域、SLA —— 过滤而非评分(策略裁剪候选集,度量在候选集内选优)
  allowed_servers[], max_hops  # 渲染成服务器侧"允许的下一跳集合"(凭据即访问声明)
  ranking_period?              # 控制平面重算 ranked list 的周期(调整周期与阻尼)
                               # address_axis = from_request 时无意义
  tuning_period                # 接入节点 top-N 内微调的周期(调整周期与阻尼)
  switch_threshold             # 调整周期与阻尼
  top_n                        # 下发给接入节点的候选数(决策位置由信息可得性决定)
  window, min_samples, stale_after            # 数据不足与候选集为空
  fallback                     # fail_closed | direct | last_known_good(数据不足与候选集为空)

PriceRecord                    # 声明值,与度量值分开(评分输入分两类)
  address, equivalence_class
  unit_price, tiers[], currency
  effective_from, effective_to, source

Measurement                    # 运行时状态,不进 SSOT(纯函数的三个产物)
  candidate_id, declaration_id, ts   # 按完整路径归因(决策位置由信息可得性决定)
  rtt_p50, rtt_p95, loss, jitter                    # 网络层
  first_byte_ms, bytes_in, bytes_out, conn_ms       # L4 可得(被动观测优先,但被动能看到什么由观测点决定)
  ttft, tokens_per_sec, error_rate                  # 应用层,需 L7 观测点(被动观测优先,但被动能看到什么由观测点决定)
  observation_point            # l4_tunnel | l7_gateway | sdk | endpoint
                               # 不同观测点的数据不可直接比较(被动观测优先,但被动能看到什么由观测点决定)
  observation_kind             # passive | active(active 计入探测预算)
  score

Credential                     # 凭据即访问声明 —— 接入凭据
  id, secret_ref, owner
  declaration_id
  issued_at, expires_at, revoked_at
                               # 出口凭据是另一类东西,见 ServiceAddress(出口鉴权凭据)
...
- **`Node` 与 `ServiceAddress` 是两张表,不是一张。** 前者是 Loom 管的机器,后者只是地址 —— 这条区分是全模型的地基([两类节点,加一类不是节点的东西](model.md#两类节点加一类不是节点的东西)、[目标地址](servers.md#目标地址));
- **没有 `target` 角色** —— 出口是路径上的位置,由 `RouteCandidate.server_chain` 的最后一项决定([出口不是一种节点,是路径上的一个位置](model.md#出口不是一种节点是路径上的一个位置));
- v1 `Tunnel.initiator` 由两端 `direction` 确定性迁移；目标态由每条 certified
  `LinkIntent` 独立固定 initiator/transport/purpose，节点级方向不再授权新边([发起方向、可达性与 transport 是每条边的契约](model.md#发起方向可达性与-transport-是每条边的契约)、[只渲染被显式授权的 LinkIntent](transports.md#只渲染被显式授权的-linkintent));
- `EquivalenceClass.carrier` 由成员 `access_contract` 是否同构决定 —— **`l4_direct` 是有前提的,不是默认可行**([访问契约:换地址能不能只靠 L4 完成](model.md#访问契约换地址能不能只靠-l4-完成));
- `RouteCandidate` 是排序、下发与归因的**唯一单位** —— 地址与服务器链不分开排序([决策位置由信息可得性决定](scheduling.md#决策位置由信息可得性决定));
- `PriceRecord` 与 `Measurement` 分表 —— **声明值与度量值性质不同**([评分输入分两类](scheduling.md#评分输入分两类))；
...
| 矛盾 | 出处 |
|---|---|
| 两端 `direction` 组合非法(rev↔rev、dir↔dir) | [v1 `direction` 只是迁移输入](model.md#v1-direction-只是迁移输入) |
| 手工指定 `mesh_eligible` / `Tunnel.initiator` / `uses_tun` / `capabilities` | [v1 `direction` 只是迁移输入](model.md#v1-direction-只是迁移输入)、[平台差异](../clients/ui.md#平台差异)、[数据平面职责分块；`control` 是正交只读投影](model.md#数据平面职责分块control-是正交只读投影) |
| 把角色字段写进错误的块(如 `access: {direction: …}`) | [数据平面职责分块；`control` 是正交只读投影](model.md#数据平面职责分块control-是正交只读投影),严格解码在加载阶段即拒绝 |
| 方向组合与显式 `Tunnel` 矛盾 | [v1 `direction` 只是迁移输入](model.md#v1-direction-只是迁移输入)、[只渲染被显式授权的 LinkIntent](transports.md#只渲染被显式授权的-linkintent)；当前不能假设 Headscale 已接管 |
| **把目标地址写成 `Node`** | [两类节点,加一类不是节点的东西](model.md#两类节点加一类不是节点的东西),目标不是节点 |
| `carrier: l4_direct` 但成员 `access_contract` 不同构 | [访问契约:换地址能不能只靠 L4 完成](model.md#访问契约换地址能不能只靠-l4-完成) |
| `objective: ttft` 但等价类缺少 L7 观测点 | [被动观测优先,但被动能看到什么由观测点决定](measurement.md#被动观测优先但被动能看到什么由观测点决定) |
| 有合规 `constraints` 但 `fallback ≠ fail_closed` | [数据不足与候选集为空](scheduling.md#数据不足与候选集为空) |
| `address_axis = from_request` 却配置了 `ranking_period` | [调整周期与阻尼](scheduling.md#调整周期与阻尼) |
| `egress_axis = pinned:<x>` 但 `x` 不在 `allowed_servers` 里 | [策略裁剪候选集,度量在候选集内选优](scheduling.md#策略裁剪候选集度量在候选集内选优) |
| `ControlSet` 为空、按在线数缩 quorum，或成员变化缺 old/new joint proof | [`control` 是动态节点能力，不是固定三台机器](control-rendering.md#control-是动态节点能力不是固定三台机器) |
| 未完成普通 Device Enrollment/overlay 身份就加入 ControlSet | [`control` 是动态节点能力，不是固定三台机器](control-rendering.md#control-是动态节点能力不是固定三台机器) |
| 相同 control epoch/revision 或 endpoint generation 对应不同 hash | [逻辑 SSOT 与纯函数渲染](control-rendering.md#逻辑-ssot-与纯函数渲染)、[公网 listener 与端口无中断轮换](public-endpoints.md#公网-listener-与端口无中断轮换) |
| 公网 Nginx 接收/代理 claim 或公开 control/config/report/Raft listener | [职责](servers.md#职责)、[客户端加入网络复用 certified SSOT 与发布链](identity.md#客户端加入网络复用-certified-ssot-与发布链)、[托管域名与证书](public-endpoints.md#托管域名与证书) |
| bootstrap capability 可访问 Enrollment 之外的地址/协议，或与 enrollment token 共用 | [客户端加入网络复用 certified SSOT 与发布链](identity.md#客户端加入网络复用-certified-ssot-与发布链) |
| capability 未绑定 certified record/policy/issuer authorization，或 admission QC 未绑定 exact claim body | [客户端加入网络复用 certified SSOT 与发布链](identity.md#客户端加入网络复用-certified-ssot-与发布链) |
| 公开 proof 正文含 intent/opening 或可离线枚举的 Device ID/Responsibilities/grants commitment | [客户端加入网络复用 certified SSOT 与发布链](identity.md#客户端加入网络复用-certified-ssot-与发布链)、[分发面：公开证明与私有 Device view 分离](distribution.md#分发面公开证明与私有-device-view-分离) |
| forward Device 无 FQDN/PublicAccessProfile，NAT public/local tuple 未分离，或 HY2/WG 共用 UDP tuple | [职责](servers.md#职责)、[公网 listener 与端口无中断轮换](public-endpoints.md#公网-listener-与端口无中断轮换) |
| DNS/ACME/端口副作用没有 certified intent 就执行，或对象缺席触发删除 | [Publisher/reconciler：提交与外部副作用分开](distribution.md#publisherreconciler提交与外部副作用分开) |
| 新 listener 未验证就 advertise/prefer，或 retire 时兼容/排空门槛不足 | [公网 listener 与端口无中断轮换](public-endpoints.md#公网-listener-与端口无中断轮换) |
| 同一 logical endpoint 的新旧 generation 无法共存、没有唯一 preferred，或轮换阶段读取 mutable latest 依赖 | [公网 listener 与端口无中断轮换](public-endpoints.md#公网-listener-与端口无中断轮换) |
