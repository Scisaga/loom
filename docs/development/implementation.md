...
当前先保持可验证的 N=1 基线并收口 Android 实机交付，不继续扩展动态 ControlSet。控制面 Web 页面、
只读库存与发布页保留；其管理写动作仍须接入管理员认证 v2 事务，不能恢复旧 registry/SSOT
写回调。当前真机已在保留 Keystore 身份和 LKG 的覆盖安装后完成真实 UDP DNS、HTTPS/TCP 与
私有报告，并证明 exact floor 不变、公网 egress 与认证固定出口一致及 Wi-Fi 生命周期恢复；下一门禁
是生产签名无损安装，以及轮换、撤权和失败回滚。单个公网 HY2 映射/路径失败保持外部阻碍，不能用客户端回退
或伪造健康状态掩盖。
