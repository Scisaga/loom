...
私有报告，并证明 exact floor 不变及公网 egress 与认证固定出口一致；下一门禁是生产签名无损安装，
以及生命周期、轮换、撤权和失败回滚。单个公网 HY2 映射/路径失败保持外部阻碍，不能用客户端回退
或伪造健康状态掩盖。
