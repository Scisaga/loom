...
| 私有控制服务 | `loom control bootstrap/serve/status/request`；[daemon](../../cmd/loom/control_runtime.go) | `control bootstrap` 只建立初始 N=1 authority；`serve()` 按当前 committed ControlSet 校验本机 member/Device/Raft tuple，先启动 overlay Raft peer listener，再 campaign 并启动 control API 与 loopback 浏览器入口。未取得多数的成员保持只读 follower，拒绝管理及 Enrollment 写事务。operation registry 含 `control_ping`、`create_invite`、`publish_device_config`、`advertise_bootstrap` 与已有 DNS binding 操作；邀请与设备配置发布均有管理员签名 CLI。认证迁移后的 `serve()` 从同一日志启动独立的私有 Enrollment、配置与报告 listener，挂载 admission/approval peer RPC；Enrollment 使用实际持久 CA 和 sealed artifact store。`device_config` 另提供 Device mTLS 的逐连接 capability authority 查询。运行中重新选主尚未接通；源码接通不证明生产 listener、静态镜像或正常 Enrollment 已验收。 |
