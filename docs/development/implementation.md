...
私有报告；下一门禁是 exact floor/最终出口回读、生产签名无损安装，以及生命周期、轮换、撤权和
失败回滚。单个公网 HY2 映射/路径失败保持外部阻碍，不能用客户端回退或伪造健康状态掩盖。
