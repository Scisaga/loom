...
| N=1 私有控制服务 | `loom control bootstrap/serve/status/request`；[daemon](../../cmd/loom/control_runtime.go) | `serve()` 启动 overlay control API、loopback 浏览器入口和 Raft。启动配置强制单成员；operation registry 含 `control_ping`、`create_invite`、`publish_device_config`、`advertise_bootstrap` 与已有 DNS binding 操作；邀请与设备配置发布均有管理员签名 CLI。认证迁移后的 `serve()` 从同一日志启动独立的私有 Enrollment、配置与报告 listener，挂载 admission/approval peer RPC；Enrollment 使用实际持久 CA 和 sealed artifact store。`device_config` 另提供 Device mTLS 的逐连接 capability authority 查询。源码接通不证明生产 listener、静态镜像或正常 Enrollment 已验收。 |
...
| Android 客户端 | [应用](../../clients/android/app/src)、[迁移安装](../../mobile/loomcore/migration_v2.go)、[v2 报告](../../mobile/loomcore/android_device_report_v2.go) | 原生 UI、多配置与 v2 桥接已接通；旧加入、公开报告/心跳、current 拉取和旧配置恢复已删除。历史配置提供迁移请求导出与文件导入，验证原平台签名、本机身份/floor、迁移证明后原子安装。迁移包已接通认证导出，Enrollment 已使用正式 daemon 与签发器，外部 bootstrap 和制品交付仍未接通；私有配置/报告共用 Go TLS 1.3 的内部 CA、SPKI 和精确目的校验；正式 daemon 已挂载回执 reader。v2 Android 运行配置显式使用绑定当前非 VPN Network 的 local DNS，业务 FakeIP 继续保留 FQDN 到最终出口。连接配置菜单在 VPN 停用时可导入 `.loom-config`，普通在线 reader 仍走私有控制链路。Debug 专用入口曾取得真实 UDP DNS、HTTPS/TCP 和私有报告回执，但当前选路不能识别候选级 HY2 失败，也不能保证直连失败后使用同出口 WG 中继，因此这些回执不能证明 Release 可用。当前设备允许卸载重装，不以保留旧 Keystore 身份作为验收前提；仍须在修正最小选路模型后构建并安装生产签名 Release，以一条代表性直连和一条同出口 fallback 完成业务验收，再验证必要的轮换、撤权与失败回滚。Debug、Release 与实际安装分别核对，见[交付规程](../clients/android-delivery.md)。 |
| 客户端选路 | [共享决策](../../internal/clientroute/clientroute.go)、[Android 宿主](../../clients/android/app/src/main/java/io/github/scisaga/loom/route/RouteManager.kt) | 候选生成保留了直连和境内入口→WG→同一境外出口，但正式 Android 仅把 ICMP 当入口 RTT，未接入候选级 HY2/libbox 拨号结果；状态上报把候选全部写成 `unknown`。直连一跳通常成为默认，现有链长门禁又阻止在证据不足/失败时增加一跳切到同出口中继。因而动态可用性、fallback 和状态展示尚未完成，现有相关断言还固化了错误规则。Windows 原生环境由另一台机器处理，本阶段不以交叉结果抵扣。应先按[最小模型交付法](minimal-model.md)删除错误规则和重复状态，再实现一个统一候选可用性模型；不要继续在 ICMP registry 上追加特判。 |
...
当前冻结功能扩张和生产部署，先按[最小模型交付法](minimal-model.md)重新审查相关 v2 改动：独立还原需求，
确认最小实体与单一事实来源，再把代码分为保留、删除、重做和未接通。控制面 Web 页面、只读库存与发布页
明确保留；其管理写动作仍须接入管理员认证 v2 事务，不能恢复旧 registry/SSOT 写回调。动态 ControlSet 暂停扩展。
Android 先修正授权候选与动态可用性的模型，再卸载重装生产签名 Release；不以保留当前 Keystore 或旧 Debug
回执作为门禁。某个公网 HY2 当前不可达是候选运行时事实，应由同出口中继 fallback 处理，不阻塞其他入口、
Enrollment 或代表性直连验收，也不要求人工遍历全部出口和 transport。
