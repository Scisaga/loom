...
在 `controlRuntime.serve()` 的正式生命周期内构造 Enrollment 的生产 provisioner，
使用同一认证 application 的真实 issuer、持久首次签发、设备本地密钥、配置封装与制品。
完成态事务必须同时更新网络分配和受影响设备的配置，不能只插入 Device view。
私有 device_config/device_report 已由正式 daemon 启动，继续复用这条调用链。

补齐 Bootstrap prepared → 外部验证 → certified advertise、实时 capability 更新与静态 catalog 发布，
再将正常 UI 创建/恢复/删除入口连接到管理员认证事务。按
[从当前实现迁移](../protocols/control-plane/migration.md#从当前实现迁移)接续保留的原身份与 floor，
删除余下旧 Enrollment handler、公开反代与失效配置；这些仍是本次替换的必需工作。
