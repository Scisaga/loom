...
| 动态 ControlSet | [成员账本](../../internal/controlplane/membership_ledger.go)、[joint leader](../../internal/controlplane/raft_joint_leader.go)、[提交后 QC peer](../../internal/controlplane/head_peer.go) | learner、成员和 quorum 组件存在；stable voter 只有从本机 committed Raft prefix 独立重算并耐久 apply 后才签 Head，形成的 exact QC 会经 control-peer mTLS 回灌并安装到 committed ControlSet quorum，follower 不再因只签不存而永久冻结。正式 daemon 的 N=1 路径已改用同一 voter/collector，在 Raft listener 挂载受 private-directory pin 保护的 Head vote/certification RPC，并把每条事务的不可变 reducer preimage 单独写入 add-only operation-material store、挂载同一私有 anti-entropy RPC；本地 result/phases 与邀请 token 明文不参与复制。Raft follower verifier 已能结合 anti-entropy 材料和同一 AppendEntries 内尚未 fsync 的 parent prefix 重算完整事务链，已 apply 前缀不重复执行昂贵 reducer。daemon 启动配置仍限制 N=1，尚未构造远端 Raft/QC/anti-entropy clients 或提供正常晋升/移除入口，不能称多成员控制面已接通。 |
