...
| 私有控制服务 | `loom control bootstrap/serve/status/request`；[daemon](../../cmd/loom/control_runtime.go) | `control bootstrap` 只建立初始 N=1 authority；`serve()` 按当前 committed ControlSet 校验本机 member/Device/Raft tuple，先启动 overlay Raft peer listener，再 campaign 并启动 control API 与 loopback 浏览器入口。未取得多数的成员保持只读 follower，拒绝管理及 Enrollment 写事务；合法 AppendEntries 刷新本机 election timer，失联 follower 按 member 顺序错峰重新 campaign，旧 term 立即失去写资格。operation registry 含 `control_ping`、`create_invite`、`publish_device_config`、`advertise_bootstrap` 与已有 DNS binding 操作；邀请与设备配置发布均有管理员签名 CLI。认证迁移后的 `serve()` 从同一日志启动独立的私有 Enrollment、配置与报告 listener，挂载 admission/approval peer RPC；Enrollment 使用实际持久 CA 和 sealed artifact store。`device_config` 另提供 Device mTLS 的逐连接 capability authority 查询。源码接通不证明生产 listener、静态镜像或正常 Enrollment 已验收。 |
| 初始 v2 authority 与存量迁移 | [daemon bootstrap](../../cmd/loom/control_runtime.go)、[共享 bootstrap 验证器](../../internal/wire/recovery.go) | 全新空网络的 `control bootstrap` 与已有 v1 网络的认证迁移是两个入口；已有网络不因空网络 genesis 坐标不同而重新 bootstrap。`control migrate` 验证原 SSOT/registry、当前管理员及原 owner/platform 双签后追加 `RuntimeActivationBundleV1`，耐久保存 exact 请求并恢复同一回执。不在用的独立客户端可明确暂存原身份，不能替代已加入身份、伪造 wrapping key 或成为 active v2 Device。`control export-migration` 从真实 activation/配置日志、原证书与原 signed current 导出设备迁移文件；缺材料时拒绝交付。`device_inputs` 已接通原请求/registry/服务器证书/signed current 验证、原权限投影和真实证书生成，并认证保留独立的原节点观测 CA，签发坐标取实际迁移日志；重启重试复用原结果。`control prepare-migration-input` 已接通原管理 TLS/Head/QC、真实证书与逐设备请求，生成完整 distribution preimage 并持久保存独立受限 issuer；`prepared` 从原日志与这些实际材料组装 application，校验原 authority 与可回读密钥；公网计划从现有材料生成并纳入同一迁移承诺。生产激活与设备回读仍按独立证据核对，不能从源码推导。 |
...
| Device/邀请与配置界面 | 私有 HTTPS → Unix socket → [Web UI](../../internal/webui/webui.go) → [生产依赖](../../internal/report/clients.go) | 管理员门禁已接通；旧公开 claim handler、证书签发、SSH 秘密预置、bootstrap 响应链、registry/SSOT 写回调及 `loom://enroll#` 生成器均已删除。库存暂读历史 registry/SSOT；生产依赖不再暴露旧邀请、重发、替换、暂停、删除和清理能力，对应按钮失败关闭。`control create-invite` CLI 已提交认证 v2 事务；UI 管理动作尚未接入认证 v2 事务客户端。 |
| v2 Enrollment 事务 | [私有服务](../../internal/enrollmentv2/private_service.go)、[Raft sequencer](../../internal/controlplane/enrollment_sequencer.go)、[首次签发持久化](../../internal/enrollmentv2/provisional_store.go) | 正式 `control serve` 已启动私有 Enrollment 服务，使用持久 CA、sealed artifact store 和同一认证日志；`use_loom` 及 Linux `forward` 职责组合的 reservation、首次签发、approval/completion 已接通，并原子生成客户端及承载服务器配置。fresh `forward` 先安装正式身份、私有控制链路和 fail-closed 首份配置，公网 ingress 保持空且不能 advertise；public-access preparing/验证由独立计划继续，FQDN、NAT、方向和 listener 不从邀请猜测，也不写入严格 v1 SSOT。Bootstrap advertised 后的 ingress 逐连接查询当前 capability，并以原 forward Device mTLS 只拨 certified Enrollment tuple；catalog/proof 随创建邀请发布到全部镜像。completion 取得 QC 后，daemon 将首份客户端及承载服务器配置推送到显式配置的 2–3 个静态镜像并逐对象回读 exact bytes，完成前不开放私有事务结果；启动时会幂等修复历史 completion 的缺失对象。源码链路仍需一次 fresh `forward` 生产正常 Enrollment 和宿主回读，不能由组件测试代替。 |
| v2 私有配置与报告 | [生成与封装](../../cmd/loom/control_prepare_client.go)、[device_config](../../internal/controlplane/device_config.go)、[device_report](../../internal/controlplane/device_report.go) | `control publish-client-config` 从当前认证网络生成 Linux/Android/Windows 运行制品和原 key 密文；移动/桌面客户端包含私有控制 WireGuard，正常发布同时认证承载分配；Linux 配置生产器消费当前分配并生成受限 userspace endpoint；发起端沿用原 WG key，通过现有 HY2 listener 的独立设备凭据承载，daemon 使用认证回环代理，reader 验证两端公钥、承载身份和精确放行/默认拒绝规则；本机真实 sing-box 隧道测试覆盖 HY2/WG 组合、两个允许服务、未授权端口和外层绕过拒绝，实际部署与验收按独立回执核对；数据面、selector 与 Agent 候选共同按当前 Device grants 收窄，出口授权只限制链尾，保留必要中继；缺授权时拒绝生成；与 `publish-device-config` 共用管理员签名、Device generation CAS、Raft/QC 和重启重放。凭据受 parent 认证 policy、真实 availability 证据及原 wrapping key 约束。认证迁移后由正式 `serve()` 加载实际专用封装 TLS 密钥、配置 reader、报告验签/持久 store 与已有观测回读，绑定独立私有 tuple；任一材料或绑定失败则整体启动失败。测试覆盖迁移身份通过完整 mTLS 链读取配置、报告持久化及重启后的序号续接。Linux `client serve-v2` 已接入配置同步、runtime 事务、持久报告和原始观测交接；服务器观测沿原周期和签名协议生成，保留原 WireGuard 接口身份，由本机缓存交给 `node-health` 私有报告，回执从已持久报告中按当前职责/原 key/新鲜度筛选，不再读取旧报告 socket；真实 TLS 测试覆盖验签、持久保存与重启续接；Linux 原服务退役已接入迁移部署事务：绑定旧文件摘要，在同一回滚边界停用并删除旧入口；报告身份读取与完整配置历史重算已分离；认证日志缓存绑定完整日志和当前 Head/QC，读前仍检查提交、认证及待处理事务边界。新入网客户端自生成 WireGuard key，reservation/provisional/completion 在同一认证日志中固化首份证书、控制链路与密文；completion 同时更新客户端与承载服务器配置，失败重启复用原结果。Linux `forward` 在公网计划尚未认证时只运行私有控制链路，后续配置继续复用 claim 固定的本机 WG 公钥，不生成或监听猜测的数据入口。未完成事务占用受影响服务器配置，到认证 retry deadline 后由下一次业务提交以 Raft/QC 终止并保留 token 占用和签发记录。生产仍需用真实 fresh `forward` 完成宿主安装、私有配置回读及 preparing 状态验收。部署与业务验收按独立回执核对。 |
| Bootstrap ingress | [HY2/Trojan 服务组件](../../internal/bootstrapaccess)、[客户端 bootstrap](../../internal/clientv2/bootstrap.go) | `certificate prepare-existing`、`control prepare/export-bootstrap` 与 `bootstrap serve` 共享认证安装计划；daemon 启动真实 HY2/Trojan socket、做本机 TLS/QUIC 验证并保存 readiness/outer plan。`bootstrap probe-outer` 生成 observer 签名报告，`control advertise-bootstrap` 在同一日志重验完整证据、持久 transition 并发布原 catalog；发布前邀请与 capability 均 fail closed。listener 从已迁移 forward Device 状态取得 private service directory、Device mTLS 与逐连接 capability resolver，Enrollment 只转发到 exact certified overlay tuple。`control create-invite` 把无 token 的 exact catalog/proof 按 typed hash 原子发布并回读全部 mirrors；completion 的配置制品复用同一不可变镜像并受 daemon 发布门禁约束。`bootstrap render-public` 从同一 bundle、证书副本和 `nginx_local_tcp_port` 生成仅 fake/immutable 的 Nginx 配置，正式源码不再依赖按节点手写的动态公网配置。源码能力仍须用精确部署提交、公网 HY2/Trojan/静态下载和一次正常 Enrollment 生产回读证明；缺这些证据时 #8 不能关闭。 |
...
| 动态 ControlSet | `loom control prepare-control-candidate`、[成员账本](../../internal/controlplane/membership_ledger.go)、[joint leader](../../internal/controlplane/raft_joint_leader.go)、[提交后 QC peer](../../internal/controlplane/head_peer.go) | learner、成员和 quorum 组件存在；stable voter 只有从本机 committed Raft prefix 独立重算并耐久 apply 后才签 Head，形成的 exact QC 会经 control-peer mTLS 回灌并安装到 committed ControlSet quorum，follower 不再因只签不存而永久冻结。正式 daemon 已按 private directory 为全部 committed remote 构造带 endpoint failover 的 Raft、Head QC 与 replication-material mTLS clients；每条 data-bearing Head 在 Raft append 前先把不可变 reducer preimage 同步到多数，follower 再结合同一 AppendEntries 内尚未 fsync 的 parent prefix 独立重算。每个 certified Head 的 exact QC 作为独立 add-only 对象复制；follower 按历史 Raft index 安装 QC，重建 operation journal、管理身份和 Enrollment 投影，本地阶段记录与邀请 token 明文不参与复制。单成员硬门禁已删除，peer listener 在 campaign 前启动，未当选进程保持 fail-closed follower；近期 leader contact 阻止 pre-vote，超时后错峰重新选主。learner storage 可按 exact old authority 重启恢复，old voter 可经非对称 old-client/new-server mTLS 向其推送 replication material 和 committed Raft prefix，learner 仍不进入投票或提交门槛。candidate 可在 active Linux Device 本机生成三类独立 control key、peer 证书和绑定当前 Device/Head 的私有 request；peer 私钥只封装给原 Device wrapping key，并提供 peer PoP 及 Device identity 签名的本机 availability receipt。服务端 candidate 校验和 learner-only 安装、catch-up checkpoint 回执以及 Joint/Final 管理事务尚未接到 daemon，因此仍不能称动态多成员控制面已完成。 |
