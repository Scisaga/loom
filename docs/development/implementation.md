...
| 浏览器管理界面 | [静态前端](../../internal/webui/static)、[JSON 业务接口](../../internal/webui/browser.go)、[结构化实时推送](../../internal/webui/device_inventory_live.go) | 顶层页面由浏览器渲染；设备职责筛选、条件表单和增量更新已接通。旧 SSR 页面及 HTML 推送已删除。流量复用已有 WireGuard 证据，不提供缺失的客户端应用流量。 |
