...
先按[最小模型交付法](minimal-model.md)写明需求、最小模型与最小必要测试集，再读
[AGENTS.md](../../AGENTS.md) 的完成判定、[源码能力对照](implementation.md)，最后读
...
1. 检查工作区和未提交改动，保护其他任务的工作。先独立写出当前要求的最小权威模型，确认每个新增
   持久状态和门禁都不可由已有状态推导；再将完成条件对应到正常 UI/CLI 入口、daemon 启动、业务处理、
   持久状态、客户端消费、部署配置和旧路径。不得按验收矩阵逐项生成一套并行状态或处理分支。
