...
- 交付证据须对应实际运行的精确提交和制品，核对生效配置，并覆盖正常成功请求、持久结果/客户端生效及
  ...
  真实部署记录只写忽略的 `docs/status/`；`current.md` 明确剩余缺口，交接必须给出下一处实际接线工作。
- 遇到缺口继续处理；确实需要外部条件时说明具体阻碍、已经完成的工作和仍未完成的功能，不用宽泛的
  “待收尾”掩盖核心缺失。更新文档和提示词本身不能代替继续实现。

控制面任务使用 [实施与交接提示词](docs/control-plane-implementation-prompt.md)。
...
只修改 Windows 客户端、必要的跨平台客户端包，以及用户明确要求的服务端验签/在线投影；
不得顺手修改 Android/Linux producer。中控用于既有加入流程和验收，不在本环境修改部署配置
或手工修补 SSOT/registry。声称完成上报实机验收时，必须取得
...
| `internal/webui/` | 浏览器只经指定 control 的 private HTTPS 进入：无客户端证书时给脱敏只读页，只有 certified Admin ACL 精确授权的 admin mTLS leaf 才能进入配置处理器，且没有 UI 密码/Cookie；节点旧 HTTP listener 只保留机器接口、不再提供页面；原生 v2 写仍须 Raft commit/apply 后取得 replication QC；公网 Nginx 不代理控制 UI/API |
