...
//
// 被动观测优先:真实流量零额外成本、零额外暴露,
// 且比合成探测更准确。**主动探测只覆盖没有真实流量的候选,必须限频、限额。**
// 此 CLI 使用显式请求预算，不定义 Windows/Android 客户端的自动选路预算。
//
// 探测走**渲染出的探测入口**(probe-in):一个回环端口,用户名
// 区分候选,路由规则把每个用户名打到同名的候选出站上。
//
// 探测必须到达本次指定的真实目标，不能用数据面自带的固定测速目标替代，
// 否则返回的延迟和成功状态无法说明本次访问是否可用。
//
// 它测的仍是 L4 可得的量(建连 + 首字节),不是 TTFT:应用层指标要 L7
// 观测点。记录里如实标 observation_point=l4_tunnel。
...
func cmdProbe(args []string) error {
	...
	budget := fs.Int("budget", 200, "本次最多发多少个探测请求 —— 探测预算是硬上限")
