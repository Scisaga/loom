...
//
// **为什么需要它。** 某次部署中一条隧道出现了按五元组持续丢包的故障：配置、
// TCP、临时 UDP 与其他 WireGuard 隧道都正常，更换监听端口后恢复。这里保留诊断
// 结论，不在仓库记录实际设备、端口和持续时间。
//
// **它只算不写。** 打印出替换后的那一行,由人粘回 SSOT：SSOT 是人的声明,
// 工具给料,不代笔。这也顺带避开了两件麻烦事:
