...
- Windows 源码、构建和虚拟化原生执行都留在当前服务器；需要时使用受限的同机
  [Windows 11 测试虚拟机](docs/windows-test-vm.md)，不维护第二份开发环境。该 VM 不能
  抵扣 ARM64、真实睡眠、物理网络切换和显示硬件的实体机终验。本分支仍只保留既定
  Windows UI 产品资产，除非当前任务明确授权，不扩张其原生功能范围。DNS provider、
  DNS-01、ACME 自动签发/续期也不属于核心重建。
