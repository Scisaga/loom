...
- 本机 GitHub HTTPS 凭据可能由 VS Code AskPass/IPC 注入交互终端；agent 子进程缺少
  `GIT_ASKPASS`/`VSCODE_GIT_*` 时，先从同用户、同仓库终端安全继承这些变量，不能据此断言无凭据。
- API 凭据通过带精确 `protocol`、`host` 和仓库 `path` 的 `git credential fill` 取得；不得打印凭据，
  不得放入命令行参数，临时认证文件必须为 `0600` 并在退出时删除。
