...
internal class PresenceReporter(
    ...
    fun send(profile: ManagedProfile): Int {
        ...
        check(body.size <= 1024) { "[§16.4 在线心跳] 三字段正文超过 1 KiB" }
        val result = HttpTransport.postPresence(context, profile.reportEndpoint, body)
        check(result.status == 204) { "[§16.4 在线心跳] 服务端未接受（HTTP ${result.status}）" }
        check(result.body.isEmpty()) { "[§16.4 在线心跳] 204 携带了意外正文" }
