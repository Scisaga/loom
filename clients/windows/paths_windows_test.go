...
func TestWindowsPathsFormatServicesQualityAndReason(t *testing.T) {
	...
	rows := windowsPathsFromReport(&clientstatus.AgentState{Selections: []clientstatus.AgentSelection{
		{Declaration: "demo-service-a", Candidate: "opaque-b", Chain: []string{"demo-prefix", "demo-exit"}, UpdatedAt: "2026-09-08T00:00:00Z", Reason: "改善达到切换门槛 [decision_scope=" + scope + "]", Health: &clientstatus.AgentCandidateHealth{SelectedState: "success", SelectedSamples: 4, Candidates: 3, RecentSuccess: 2, Unknown: 1, SelectedP50MS: &p50, SelectedP95MS: &p95, BestP50MS: &zero}},
		{Declaration: "demo-service-b", Candidate: "opaque-c", Chain: []string{"demo-other"}, Reason: "当前路径发生故障", Health: &clientstatus.AgentCandidateHealth{SelectedState: "degraded", SelectedKBps: &kbps}},
...
func TestWindowsPathReaderLimitsReadsAndDiscardsFailures(t *testing.T) {
	...
	reads := 0
	fail := false
	reader := &windowsPathReader{read: func(context.Context, clientRuntimeState, time.Time) (*clientstatus.AgentState, error) {
		reads++
		if fail {
			return nil, errors.New("demo API unavailable")
		}
		return &clientstatus.AgentState{Selections: []clientstatus.AgentSelection{{Declaration: "demo-service", Candidate: "opaque-b", Chain: []string{"demo-prefix-b", "demo-exit"}}}}, nil
	}}
	now := time.Now()
...
func TestWindowsPathsDiscardGenerationChangedDuringRead(t *testing.T) {
	for _, change := range []string{"cancel", "replace", "disconnect"} {
		t.Run(change, func(t *testing.T) {
			root, control, cancel := windowsPathControlFixture(t)
			reader := &windowsPathReader{read: func(ctx context.Context, state clientRuntimeState, at time.Time) (*clientstatus.AgentState, error) {
				switch change {
				case "cancel":
					cancel()
				case "replace":
				...
				return &clientstatus.AgentState{Selections: []clientstatus.AgentSelection{{Declaration: "demo-service", Candidate: "opaque-old", Chain: []string{"demo-old"}}}}, nil
			}}
...
func TestWindowsPathsShowProbeCountsAndIncompleteComparison(t *testing.T) {
	...
	rows := windowsPathsFromReport(&clientstatus.AgentState{Selections: []clientstatus.AgentSelection{{Declaration: "demo-service", Health: health, UpdatedAt: "demo-read-time"}}})
	row := rows[0]
	...
	row = windowsPathsFromReport(&clientstatus.AgentState{Selections: []clientstatus.AgentSelection{{Health: health}}})[0]
...
func TestWindowsPathsDescribeEntryAndServerEvidence(t *testing.T) {
	...
	rows := windowsPathsFromReport(&clientstatus.AgentState{Selections: []clientstatus.AgentSelection{{Declaration: "demo-service", Candidate: "demo-path", Chain: []string{"demo-entry", "demo-exit"}, Reason: reason}}})
